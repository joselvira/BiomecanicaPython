...
try: #para que intente cargar antes la versión de mi carpeta más actualizada
    ...
    sys.path.append(r'C:\Users\Public\Documents\Vicon\Nexus2.x\ModelTemplates\Functions')
    from readViconCsv import read_vicon_csv, read_vicon_csv_pl_xr
    from read_vicon_c3d import read_vicon_c3d_xr
...
# %% Funciones varias
# =============================================================================
def PresentaBase(SubjectName, origen, matrizRot, nombre, exists, escala):
    ...
    vicon.SetModelOutput(SubjectName, modeledName, (origen).T, exists)
    
    modeledName = 'Base'+nombre+'X'
    if modeledName not in vicon.GetModelOutputNames(SubjectName):
        vicon.CreateModeledMarker(SubjectName, modeledName)
    vicon.SetModelOutput(SubjectName, modeledName, ((matrizRot[0]*escala)+origen).T, exists)
    
    modeledName = 'Base'+nombre+'Y'
    if modeledName not in vicon.GetModelOutputNames(SubjectName):
        vicon.CreateModeledMarker(SubjectName, modeledName)
    vicon.SetModelOutput(SubjectName, modeledName, ((matrizRot[1]*escala)+origen).T, exists)
    
    modeledName = 'Base'+nombre+'Z'
    if modeledName not in vicon.GetModelOutputNames(SubjectName):
        vicon.CreateModeledMarker(SubjectName, modeledName)
    vicon.SetModelOutput(SubjectName, modeledName, ((matrizRot[2]*escala)+origen).T, exists)
...
def normaliza_vectores(x, y , z):
    ...
    x = xr.apply_ufunc(normaliza_vector_aux, x,
                       input_core_dims=[['axis', 'time']],
                       output_core_dims=[['time', 'axis']],
                       dask='parallelized',
                       vectorize=True,
                       )
    y = xr.apply_ufunc(normaliza_vector_aux, y,
                    input_core_dims=[['axis', 'time']],
                    output_core_dims=[['time', 'axis']],
                    dask='parallelized',
                    vectorize=True,
                    )
    z = xr.apply_ufunc(normaliza_vector_aux, z,
                    input_core_dims=[['axis', 'time']],
                    output_core_dims=[['time', 'axis']],
                    dask='parallelized',
                    vectorize=True,
...
def calcula_bases(daData, modelo_completo=False):
    """
    Calcula las matrices de rotación de cada segmento a partir de sus marcadores.
    Recibe trayectorias marcadores ya separadas en lados.
    """
    ...
    try:
        origen = (daData.sel(n_var='ASI', side='L') + daData.sel(n_var='ASI', side='R'))*0.5
        x = daData.sel(n_var='ASI', side='R') - origen
        vprovis = origen - (daData.sel(n_var='PSI', side='L') + daData.sel(n_var='ASI', side='R'))*0.5
        z = xr.cross(x, vprovis, dim='axis')
        y = xr.cross(z, x, dim='axis')
        
        x, y, z = normaliza_vectores(x, y, z)

        RlG = xr.concat([x, y, z], dim='axis_base')#.transpose('ID', 'axis_base', 'time', 'axis')
    ...
    try:
        ...
        vprovis = daData.sel(n_var='KneeInt', side='L') - daData.sel(n_var='KneeExt', side='L')
    ...
    try:
        ...
        vprovis = daData.sel(n_var='KneeExt', side='R') - daData.sel(n_var='KneeInt', side='R')
        y = xr.cross(z, vprovis, dim='axis')
        x = xr.cross(y, z, dim='axis')
        x, y, z = normaliza_vectores(x, y, z)
    ...
    try:
        ...
        vprovis = daData.sel(n_var='AnkleInt', side='L') - daData.sel(n_var='AnkleExt', side='L')
        y = xr.cross(z, vprovis, dim='axis')
        x = xr.cross(y, z, dim='axis')
        x, y, z = normaliza_vectores(x, y, z)
    ...
    try:
        ...
        vprovis = daData.sel(n_var='AnkleExt', side='R') - daData.sel(n_var='AnkleInt', side='R')
        y = xr.cross(z, vprovis, dim='axis')
        x = xr.cross(y, z, dim='axis')
        x, y, z = normaliza_vectores(x, y, z)
    ...
    try:
        ...
        y = daData.sel(n_var='Meta', side='L') - daData.sel(n_var='TalonSup', side='L')
        vprovis = daData.sel(n_var='TalonSup', side='L') - daData.sel(n_var='TalonInf', side='L')
    ...
    try:
        ...
        y = daData.sel(n_var='Meta', side='R') - daData.sel(n_var='TalonSup', side='R')
        vprovis = daData.sel(n_var='TalonSup', side='R') - daData.sel(n_var='TalonInf', side='R')
        x = xr.cross(y, vprovis, dim='axis')
        z = xr.cross(x, y, dim='axis')
        x, y, z = normaliza_vectores(x, y, z)
    ...
    if modelo_completo:
        #----LUMBAR_LR
        try:
            origen = (daData.sel(n_var='PSI', side='L') + daData.sel(n_var='PSI', side='R')) * 0.5
        ...
        try:
            ...
            vprovis = daData.sel(n_var='Hombro', side='R') - daData.sel(n_var='Hombro', side='L')
            y = xr.cross(z, vprovis, dim='axis')
            x = xr.cross(y, z, dim='axis')
            x, y, z = normaliza_vectores(x, y, z)
        ...
        try:
            ...
            y = daData_completo.sel(n_var='Ant_Cabeza') - daData_completo.sel(n_var='Post_Cabeza')
            #DÓNDE SE CARGA LADOS DE CABEZA????
            vprovis = daData_completo.sel(n_var='Cabeza', side='R') - daData_completo.sel(n_var='Cabeza', side='L')
    ...
    return dsRlG.drop_vars(['n_var', 'side']) 


def calcula_angulos_segmentos(dsRlG, verbose=False):
    dsAngSeg = xr.Dataset()

    for RlG in dsRlG:
        if verbose: print(RlG)
        dsAngSeg[f'AngSeg{RlG.upper()}'] = calcula_angulo_xr(RlGChild=dsRlG[RlG])
    #RlGChild=dsRlG[RlG]
    return dsAngSeg


def calcula_angulos_articulaciones(dsRlG, modelo_completo=False, verbose=False):
    """
    ...
    """
    modeled_names = ['AngArtHip_L', 'AngArtHip_R', 'AngArtKnee_L', 'AngArtKnee_R', 'AngArtAnkle_L', 'AngArtAnkle_R', ]
    child_parents = [['Muslo_L', 'Pelvis_LR'],
                     ['Muslo_R', 'Pelvis_LR'],
                     ['Pierna_L', 'Muslo_L'],
                     ['Pierna_R', 'Muslo_R'],
                     ['Retropie_L', 'Pierna_L'],
    ...
    if modelo_completo:
        modeled_names += ['AngArtLumbar_LR', 'AngArtToracoLumbar_LR',
                         #'AngArtL1_LR', 'AngArtT6_LR',
                         'AngArtCuello_LR',
                         #'AngArtElbow_L', 'AngArtElbow_R',
                         ]
        child_parents += [['Pelvis_LR', 'Lumbar_LR'],
                         ['Lumbar_LR', 'Torax_LR'],
                         ['Torax_LR', 'Cabeza_LR'],                         
                        ]
    ...
    dsAngles['AngArtHip_R'].loc[dict(axis='y')] = -dsAngles['AngArtHip_R'].loc[dict(axis='y')] #invierte el signo de la abd-add
    dsAngles['AngArtHip_R'].loc[dict(axis='z')] = -dsAngles['AngArtHip_R'].loc[dict(axis='z')] #invierte el signo de la rot
    #KNEE
    dsAngles['AngArtKnee_L'].loc[dict(axis='x')] = -dsAngles['AngArtKnee_L'].loc[dict(axis='x')] #solo se invierte el signo de la flexoext        
    dsAngles['AngArtKnee_R'] *= -1 #dsAngles[modeled_name].loc[dict(axis=['x', 'y', 'z'])
    #ANKLE
    dsAngles['AngArtAnkle_L'].loc[dict(axis='y')] = -dsAngles['AngArtAnkle_L'].loc[dict(axis='y')] #solo se invierte el signo de la pronosup tobillo izq
    dsAngles['AngArtAnkle_R'].loc[dict(axis='z')] = -dsAngles['AngArtAnkle_R'].loc[dict(axis='z')] #solo se invierte el signo de la rotación tobillo der
    ...
    if modelo_completo:        
        ...
        try:
            ...
            p1 = daDatos_completo.sel(n_var='T6')
            p2 = daDatos_completo.sel(n_var='L1')
            p3 = (daDatos.sel(n_var='PSI', side='L') + daDatos.sel(n_var='PSI', side='R')) * 0.5
        ...
        try:
            #Probar con esto en xr, si no, hacer función aux y usar ufunc
            p1 = daDatos_completo.sel(n_var='C7')
            p2 = daDatos_completo.sel(n_var='T6')
            p3 = daDatos_completo.sel(n_var='L1')
            dsAngles[modeled_name] = 180 - (np.arctan2(np.linalg.norm(xr.cross(p1-p2, p3-p2, dim='axis')), xr.dot(p1-p2, p3-p2, dim='axis'))*180/np.pi)
        ...
        try:
            #Probar con esto en xr, si no, hacer función aux y usar ufunc
            p1 = daDatos.sel(n_var='Muneca', side='L')
            p2 = daDatos.sel(n_var='Codo', side='L')
            p3 = daDatos.sel(n_var='Hombro', side='L')
            dsAngles[modeled_name] = 180 - (np.arctan2(np.linalg.norm(xr.cross(p1-p2, p3-p2, dim='axis')), xr.dot(p1-p2, p3-p2, dim='axis'))*180/np.pi)
        ...
        try:
            #Probar con esto en xr, si no, hacer función aux y usar ufunc
            p1 = daDatos.sel(n_var='Muneca', side='R')
            p2 = daDatos.sel(n_var='Codo', side='R')
            p3 = daDatos.sel(n_var='Hombro', side='R')
            dsAngles[modeled_name] = 180 - (np.arctan2(np.linalg.norm(xr.cross(p1-p2, p3-p2, dim='axis')), xr.dot(p1-p2, p3-p2, dim='axis'))*180/np.pi)
...
def ajusta_etiquetas_lado_final(daDatos):
    labels = daDatos['n_var'].values
     #Ajusta las etiquetas a formato lados L, R
    n_var_nuevo = [i.split('Left_')[-1] + '_L' if 'Left' in i else i for i in labels]
    n_var_nuevo = [i.split('Right_')[-1] + '_R' if 'Right' in i else i for i in n_var_nuevo]
    n_var_nuevo = [i[1:] + '_L' if i[0]=='L' else i for i in n_var_nuevo]
    n_var_nuevo = [i[1:] + '_R' if i[0]=='R' else i for i in n_var_nuevo]

    daDatos = daDatos.assign_coords(n_var=n_var_nuevo)
    return daDatos

def traduce_variables(nomvar):
    if nomvar=='GLU':
        nom=u'Glúteo'
    elif nomvar=='BIC':
        nom=u'Bíceps femoral'
    elif nomvar=='REC':
        nom=u'Recto femoral'
    elif nomvar=='VME':
        nom=u'Vasto interno'
        ...
        nom=u'Velocidad Ang biela'    
    else:
        nom=u'desconocido'
    return nom

def calcula_angulo_xr(RlGChild, RlGParent=None):
    """
    ...
    Rxr[0,0]
    """
    angulo = xr.apply_ufunc(calc_ang_aux, RlGChild, RlGParent,
                   input_core_dims=[['axis_base', 'axis'], ['axis_base', 'axis']],
                   output_core_dims=[['axis']],
...
# =============================================================================
def carga_variables_nexus_Trajectories(vicon=None, n_vars=None):
    ...
    for nom in n_vars:
        ...
        if dat.shape[1]==0:
            dat = np.array([vicon.GetModelOutput(vicon.GetSubjectNames()[0], nom)[0][:3]])[0] #.reshape(3, vicon.GetTrialRange()[1]).T    
    ...
    #data = np.expand_dims(np.array(coords, dtype=object), axis=0) #añade una dimensión para el ID
    data = np.expand_dims(np.array(markers), axis=0) #añade una dimensión para el ID
...
def carga_variables_nexus_force(vicon=None, n_plate=None):
...
def carga_variables_nexus_EMG(vicon=None, n_vars=None):
    ...
    #Coge los nombres de los canales
    cols = [vicon.GetDeviceOutputDetails(deviceID,outputIDs[i])[0] for i in range(len(outputIDs))]
...
# =============================================================================
def pasa_df_a_da_EMG(data):
    #Pasa de df a da
    if isinstance(data, pd.DataFrame):
        da = (data.set_index(['ID', 'time'])
                   .stack()
                   .to_xarray()
    ...
    L = da.sel(n_var=da.n_var.str.endswith('_L'))#[i for i in da.n_var.data if '_L' in i and '_LR' not in i])#nomVarsContinuas_L) #el L es distinto porque incluye _L y _LR 
    ...
    da = xr.concat([L, R], pd.Index(['L','R'], name='side')).transpose('ID', 'n_var', 'side', 'time')
    return da
...
def carga_trayectorias_c3d(lista_archivos, nom_vars_cargar=None):
    raise Exception("Deprecation warning. Mejor utilizar carga_c3d_generico_xr(data, section='Trajectories')")
    #return
    ...
    daTodosArchivos = [] #guarda los dataframes que va leyendo en formato lista y al final los concatena
    ...
    daTodosArchivos = xr.concat(daTodosArchivos, dim='ID')#.transpose('ID', 'n_var', 'side', 'axis', 'time')
               
    print('Cargados {0:d} archivos en {1:.3f} s \n'.format(numArchivosProcesados, time.time()-timer))
...
def separa_trayectorias_lados(daData):    
    L = daData.sel(n_var=daData.n_var.str.endswith('_L'))#[i for i in da.n_var.data if '_L' in i and '_LR' not in i])#nomVarsContinuas_L) #el L es distinto porque incluye _L y _LR 
    R = daData.sel(n_var=daData['n_var'].str.endswith('_R'))
    LR = daData.sel(n_var=daData['n_var'].str.endswith('_LR')) #daTodosArchivos.sel(n_var=list(daTodosArchivos.n_var[daTodosArchivos['n_var'].str.endswith('_LR').data].data))
    ...
    L = xr.concat([L, LR], dim='n_var') #.transpose('ID', 'n_var', 'side', 'axis', 'time')
    ...
    daData = xr.concat([L, R], pd.Index(['L','R'], name='side')).transpose('ID', 'n_var', 'side', 'axis', 'time')
...
# =============================================================================
def carga_csv_generico_pl_xr(lista_archivos, nom_vars_cargar=None, section=None):
    #Versión con Polars
    ...
    for nf, file in enumerate(lista_archivos[:]): 
        ...
        try:
            ...
            daProvis = (read_vicon_csv_pl_xr(file, section=section,
                        ...
                        .expand_dims({'ID':[f"{file.parent.parts[-2].replace('_','')}_{'_'.join(file.stem.split('-'))}"]},
    ...
    print(f'Cargados {num_archivos_procesados} archivos en {time.perf_counter()-timerCarga:.3f} s \n')
...
def carga_c3d_generico_xr(lista_archivos, nom_vars_cargar=None, section=None):
    print('\nCargando los archivos...')
    timerCarga = time.perf_counter() #inicia el contador de tiempo
    ...
    for nf, file in enumerate(lista_archivos[:]): 
        ...
        try:
            ...
            daProvis = (read_vicon_c3d_xr(file, section=section,
                                              n_vars_load=nom_vars_cargar)
                        .expand_dims({'ID':[f"{file.parent.parts[-2].replace('_','')}_{'_'.join(file.stem.split('-'))}"]},
                                      axis=0) #Añade columna ID
                        )
    ...
    daTodos = (xr.concat(daTodos, dim='ID', coords='minimal')
               .astype('float')
               )
...
def carga_preprocesa_csv_cinem(listaArchivos, nom_vars_cargar=None, nomArchivoPreprocesado=None):
    ...
    renombrar_vars_coords={'AngArtLHip_x':'AngArtHip_L_x', 'AngArtRHip_x':'AngArtHip_R_x', 'AngArtLHip_y':'AngArtHip_L_y', 'AngArtRHip_y':'AngArtHip_R_y', 'AngArtLHip_z':'AngArtHip_L_z', 'AngArtRHip_z':'AngArtHip_R_z', 
                           'AngArtLKnee_x':'AngArtKnee_L_x', 'AngArtRKnee_x':'AngArtKnee_R_x', 'AngArtLKnee_y':'AngArtKnee_L_y', 'AngArtRKnee_y':'AngArtKnee_R_y', 'AngArtLKnee_z':'AngArtKnee_L_z', 'AngArtRKnee_z':'AngArtKnee_R_z', 
                           'AngArtLAnkle_x':'AngArtAnkle_L_x', 'AngArtLAnkle_y':'AngArtAnkle_L_y', 'AngArtLAnkle_z':'AngArtAnkle_L_z', 'AngArtRAnkle_x':'AngArtAnkle_R_x', 'AngArtRAnkle_y':'AngArtAnkle_R_y', 'AngArtRAnkle_z':'AngArtAnkle_R_z', 
                           'AngArtLumbar_x':'AngArtLumbar_LR_x', 'AngArtLumbar_y':'AngArtLumbar_LR_y', 'AngArtLumbar_z':'AngArtLumbar_LR_z',
                           'AngSegPELVIS_x':'AngSegPELVIS_LR_x', 'AngSegPELVIS_y':'AngSegPELVIS_LR_y', 'AngSegPELVIS_z':'AngSegPELVIS_LR_z', 
                           'AngSegTORAX_x':'AngSegTORAX_LR_x', 'AngSegTORAX_y':'AngSegTORAX_LR_y', 'AngSegTORAX_z':'AngSegTORAX_LR_z', 
                           'AngArtCuello_x':'AngArtCuello_LR_x', 'AngArtCuello_y':'AngArtCuello_LR_y', 'AngArtCuello_z':'AngArtCuello_LR_z',  
                           'AngArtL1_x':'AngArtL1_LR_x',	'AngArtL1_y':'AngArtL1_LR_y',
                           'AngBiela_y':'AngBiela_LR_y', 'vAngBiela':'vAngBiela_LR_x',
                           }
    renombrar_vars={'AngArtLHip':'AngArtHip_L', 'AngArtRHip':'AngArtHip_R', 
                    'AngArtLKnee':'AngArtKnee_L', 'AngArtRKnee':'AngArtKnee_R',
                    'AngArtLAnkle':'AngArtAnkle_L', 'AngArtRAnkle':'AngArtAnkle_R',
                    ...
                    'AngBiela':'AngBiela_LR', 'vAngBiela':'vAngBiela_LR',
                    'LHJC':'HJC_L', 'RHJC':'HJC_R',
                    'LKJC':'KJC_L', 'RKJC':'KJC_R',
                    'LAJC':'AJC_L', 'RAJC':'AJC_R',
                    'LPosPedal': 'PosPedal_L', 'RPosPedal': 'PosPedal_R',
                    'X':'x', 'Y':'y', 'Z':'z'
                    }
    ...
    for file in listaArchivos:
        ...
        try:
            ...
            dfprovis = pd.concat([dfprovis, dfprovis[['AngBiela_LR']].rename(columns={'AngBiela_LR': 'AngBiela_L'}), dfprovis[['AngBiela_LR']].rename(columns={'AngBiela_LR': 'AngBiela_R'})], axis=1)
            
            #Ajusta el AngBiela R para ser como el L pero con diferencia de 180º
            dfprovis.loc[:, ('AngBiela_R', 'x')] = dfprovis.loc[:, ('AngBiela_R', 'x')].where(dfprovis.loc[:, ('AngBiela_R', 'x')] < np.pi, dfprovis.loc[:, ('AngBiela_R', 'x')]-2*np.pi)+np.pi
            dfprovis.loc[:, ('AngBiela_R', 'y')] = dfprovis.loc[:, ('AngBiela_R', 'x')] - np.pi #dfprovis.loc[:, ('AngBiela_R', 'y')].where(dfprovis.loc[:, ('AngBiela_R', 'y')]<0.0, dfprovis.loc[:, ('AngBiela_R', 'y')]-2*np.pi)+np.pi
            dfprovis.loc[:, ('AngBiela_R', 'z')] = dfprovis.loc[:, ('AngBiela_R', 'z')].where(dfprovis.loc[:, ('AngBiela_R', 'z')] < 0.0, dfprovis.loc[:, ('AngBiela_R', 'z')]-2*np.pi)+np.pi
    ...
    daTodos = (dfTodosArchivos.set_index(['ID', 'time'])
               .stack()
               .to_xarray().to_array()
               .rename({'variable':'n_var'})
              )#.transpose('ID', 'n_var', 'axis', 'time')
    ...
    L = daTodos.sel(n_var=daTodos.n_var.str.endswith('_L'))#[i for i in da.n_var.data if '_L' in i and '_LR' not in i])#nomVarsContinuas_L) #el L es distinto porque incluye _L y _LR 
    R = daTodos.sel(n_var=daTodos['n_var'].str.endswith('_R'))
    LR = daTodos.sel(n_var=list(daTodos.n_var[daTodos['n_var'].str.endswith('_LR').data].data))
    ...
    daTodos = xr.concat([L, R, LR], pd.Index(['L','R', 'LR'], name='side')).transpose('ID', 'n_var', 'side', 'axis', 'time')
    try:
        daTodos.loc[dict(n_var=['HJC', 'KJC', 'AJC'])] /= 10 #pasa las posiciones de los ejes a cm
    except:
        pass
    #daTodos.isel(ID=0).sel(n_var='AngBiela')
    
    #Calcula vAngBiela
    vAngBiela = (xr.apply_ufunc(np.unwrap, daTodos.sel(n_var='AngBiela')).differentiate(coord='time')
                .expand_dims(dim=['n_var'])
                .assign_coords(dict(n_var=['vAngBiela']))
              )
    vAngBiela = np.rad2deg(vAngBiela) #pasa de radianes a grados
    daTodos = xr.concat([daTodos, vAngBiela], dim='n_var', join='left') #join left para que guarde el orden de coords lado
    ...
    if False:
        vAngBiela.sel(n_var='vAngBiela').plot.line(x='time', row='ID', col='side', hue='axis', sharey=False)
        daTodos.sel(n_var='vAngBiela').plot.line(x='time', row='ID', col='side', hue='axis', sharey=False)
       
        daTodos.isel(ID=slice(0,-1)).sel(n_var='AngArtKnee', axis='x', side=['L', 'R']).plot.line(x='time', col='side')
        
        daTodos.isel(ID=slice(0,-1)).sel(n_var='AngBiela', side='LR').plot.line(x='time', col='axis', hue='ID')
        daTodos.isel(ID=slice(2,4)).sel(n_var='vAngBiela', side='LR').plot.line(x='time', col='axis', hue='ID', sharey= False)
    #-------------------------------
...
def carga_preprocesa_c3d_cinem(listaArchivos, nom_vars_cargar=None):
    import c3d
    ...
    for file in listaArchivos:
        ...
        try:
            ...
            da = xr.DataArray(data=np.expand_dims(data, axis=0) / 10,
                              dims=('ID', 'n_var', 'time'),
                              coords={'ID': [file.parent.parts[-2]+'_'+file.stem],
                                      'n_var': (labels),
            ...
            da = da.assign_coords(n_var=(['GLU_R', 'BIC_R', 'REC_R', 'VME_R', 'GAS_R', 'TIB_R','GLU_L', 'BIC_L', 'REC_L', 'VME_L', 'GAS_L', 'TIB_L']))
    ...
    for file in listaArchivos:
        ...
        try:
            ...
            if nomBloque=='Devices':
                ...
                dfprovis = (dfprovis.rename(columns=renombrar_vars).sort_index()
    ...
    for file in listaArchivos:
        ...
        try:
            ...
            with open(file, 'rb') as handle:
                ...
                labels = [s.split('.')[0].replace(' ', '') for s in reader.analog_labels]
...
def procesaEMG(daEMG, fr=None, fc_band=[10, 400], fclow=8, btkeo=False):
    from filtrar_Butter import filtrar_Butter, filtrar_Butter_bandpass
    
    if fr==None:
        fr=daEMG.freq
    #Filtro band-pass
    daEMG_proces = filtrar_Butter_bandpass(daEMG, fr=fr, fclow=fc_band[0], fchigh=fc_band[1])
    #Centra, ¿es necesario?
    daEMG_proces = daEMG_proces - daEMG_proces.mean(dim='time')
    
    if btkeo:
        daEMG_proces = xr.apply_ufunc(tkeo, daEMG_proces,
...
if __name__ == "__main__":               
    if False:
        ...
        daDatos = carga_c3d_generico_xr(lista_archivos[:10], section='Trajectories')  #, nom_vars_cargar=['HJC', 'KJC', 'AJC'])
