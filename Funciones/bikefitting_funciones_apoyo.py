...
bEnsembleAvg = True #puede ser True, False o 'completo' (la media con cada repe de fondo)
...
sys.path.append('/media/yomismo/Seagate Basic/Programacion/Python/Mios/Functions') #para Linux
...
def calcula_angulos_desde_trajec(daData, modelo_completo=False, verbose=False):
    ...
    dsAngArtics = nfa.calcula_angulos_articulaciones(dsRlG, modelo_completo, verbose=verbose)
    #dsAngArtics['AngArtHip_L'].plot.line(x='time', row='ID', hue='axis')
    
    #Calcula ángulo de biela. Primero ejes pedales
    daEjePedal = (daData.sel(n_var=['Pedal_A', 'Pedal_P']).mean(dim='n_var')
                    .expand_dims({'n_var':['PosPedal']},
                                      axis=0) #Añade dimensión n_var
                  )
    #Si hace falta, por aquí seleccionar qué variables se usan para calcular angBiela (modelo_eje_biela='bilateral')
    daAngBiela = calcula_ang_biela(daEje1=daEjePedal.sel(side='R'), daEje2=daEjePedal.sel(side='L'))

    #Calcula velocidad angular biela
    davAngBiela = (xr.apply_ufunc(np.unwrap, daAngBiela).differentiate(coord='time')
                    #.expand_dims(dim=['n_var'])
                    #.assign_coords(dict(n_var=['vAngBiela']))
                  )
    davAngBiela = np.rad2deg(davAngBiela) #pasa de radianes a grados
    davAngBiela.name = 'vAngBiela_LR'
    #davAngBiela.plot.line(x='time', row='ID', hue='axis')
    

    daAngles = (xr.merge([dsAngSegments, dsAngArtics, daAngBiela, davAngBiela])
                .to_dataarray()
    ...
    if 'side' in daData.coords:
        daAngles = nfa.separa_trayectorias_lados(daAngles)
...
def calcula_ang_biela(daEje1, daEje2):
    """
    ...
    x = np.arctan2(daEje1.sel(axis='y') - daEje2.sel(axis='y'), daEje1.sel(axis='z') - daEje2.sel(axis='z')) + np.pi  #cero con L arriba , PI con R arriba
    y = np.arctan2(daEje1.sel(axis='y') - daEje2.sel(axis='y'), daEje1.sel(axis='z') - daEje2.sel(axis='z'))  #mínimo con L arriba, paso por cero con R arriba
    z = np.arctan2(daEje1.sel(axis='z') - daEje2.sel(axis='z'), daEje1.sel(axis='y') - daEje2.sel(axis='y'))  #mínimo con L delante, paso por cero con R delante
    
    daAngBiela = (xr.concat([x, y, z], dim='axis').assign_coords(axis=['x', 'y', 'z'])
                    # .expand_dims({'side':['LR']},
                    #                     axis=0) #Añade dimensión
                    ).squeeze('n_var').drop_vars('n_var')
    """
    x.plot.line(x='time', row='ID')
    daAngBiela.isel(side=0, n_var=0).plot.line(x='time', row='ID', col='axis')
    daAngBiela.sel(ID='17Eduardo_Circular_01').isel(side=0, n_var=0).plot.line(x='time', col='axis')
    """
    ...
    daAngBiela.attrs['units'] = 'deg'

    return daAngBiela

        

def calcula_variables_posicion(daData, modelo_eje_biela='bilateral', verbose=False):
    """
    ...
    """
    print('Calculando variables de marcadores...')
    ...
    if modelo_eje_biela == 'bilateral':
        ...
        daEjeBiela = (daEjePedal.sel(side=['L', 'R']).mean(dim=['side', 'time'])
    ...
    #TODO: #NO ADAPTADO A XARRAY
        ...
        try:
            ...
            for fot in range(RegionOfInterest[0]+100, RegionOfInterest[1]-100, 100):
                centro.append(define_circle(marcProvis[fot, 1:3], marcProvis[fot+30, 1:3], marcProvis[fot+60, 1:3]))
            centro = np.array(centro).mean(axis=0) #calcula la media de todos los centros que ha calculado
            ...
            vicon.SetModelOutput(SubjectName, modeledName, ejeBiela.T, exists)
                ...
                print('No se ha podido calcular el eje de la biela con el modelo "derecha o izquierda".')

    
    daPos = xr.concat([daEjeArtic, daEjePedal, daEjeBiela], dim='n_var', join='left') #join left para que guarde el orden de coords lado
...
def calcula_spm1d(df):
    ...
    dfProvis_L = df.query('side==@df.side.unique()[0]') #con unique para evitar problemas nomenclatura L Izq, R Der
    ...
    YL = dfProvis_L.pivot(index='AngBielaInRepe', columns='phase', values=['value']).to_numpy().T
    YR = dfProvis_R.pivot(index='AngBielaInRepe', columns='phase', values=['value']).to_numpy().T
    # plt.plot(YL.T)
    # plt.plot(YR.T)
    
    try: #si alguna variable es cero, se peta
        t  = spm1d.stats.ttest_paired(YL,YR) #YL[repes[0]:repes[1]], YR[repes[0]:repes[1]])
...
#Post hoc
def plot_clusters(spmi, y=0, ax=None, print_p=True):
...
def segmenta_ModeloBikefitting_xr_cinem(daDatos, num_cortes=12, graficas=False):
    ...
    daL = stsp(data=daDatos.sel(side='L'), func_events=stsp.detect_onset_detecta_aux,
    ...
    daR = stsp(data=daDatos.sel(side='R'), func_events=stsp.detect_onset_detecta_aux,
              reference_var=dict(n_var='AngBiela', axis='y'),
              discard_phases_ini=1, discard_phases_end=0, n_phases=num_cortes,
              include_first_next_last=True, 
    ...
    daSegment = (xr.concat([daL, daR], pd.Index(['L','R'], name='side'))
                 .transpose('ID', 'n_var', 'side', 'axis', 'phase', 'time')
                 #.isel(n_var=slice(0,-1))#quita la variable creada distancia pedales
                 )
...
def segmenta_ModeloBikefitting_xr_EMG(daDatos, num_cortes=12, graficas=False):
    #from detecta import detect_peaks
    #from cortar_repes_ciclicas import detect_onset_aux
    print(f'Segmentando {len(daDatos.ID)} archivos.')
    daL = stsp(data=daDatos.sel(side='L'), func_events=stsp.detect_onset_detecta_aux,
    ...
    #           var_referencia=dict(n_var='AngBiela'),
    #           descarta_corte_ini=1, num_cortes=num_cortes, descarta_corte_fin=0,
    #           **dict(threshold=0.0, n_above=2, corte_ini=0, show=graficas)).corta_repes()
    
    #daL = corta_repes_xr(daDatos.sel(side='L'), func_cortes=detect_onset_aux, var_referencia=dict(n_var='AngBiela'), descarta_corte_ini=1, descarta_corte_fin=0, num_cortes=num_cortes, **dict(threshold=0.0, n_above=2, corte_ini=1, show=graficas))
    #daR = corta_repes_xr(daDatos.sel(side='R'), func_cortes=detect_onset_aux, var_referencia=dict(n_var='AngBiela'), descarta_corte_ini=1, descarta_corte_fin=0, num_cortes=num_cortes, **dict(threshold=0.0, n_above=2, corte_ini=0, show=graficas))
    
    # daL.isel(ID=0).plot.line(x='time', hue='corte', row='n_var', sharey=False) 
    
    """
    #Con PosPedal
    daL = corta_repes_xr(daDatos.sel(side='L'), func_cortes=detect_peaks, var_referencia=dict(n_var='dPedal_z'), descarta_rep_ini=1, descarta_rep_fin=0, **dict(valley=True, mph=-100, mpd=50, show=graficas))
    daR = corta_repes_xr(daDatos.sel(side='R'), func_cortes=detect_peaks, var_referencia=dict(n_var='dPedal_z'), descarta_rep_ini=1, descarta_rep_fin=0, **dict(valley=False, mph=100, mpd=50, show=graficas))#.sel(side='R')
    """
    
    daSegment = (xr.concat([daL, daR], pd.Index(['L','R'], name='side'))
                 .transpose('ID', 'n_var', 'side', 'phase', 'time')
...
# =============================================================================
def normaliza_t_aux(data, x, base_norm_horiz): #Función auxiliar para normalizar con xarray
...
def NormalizaBiela360_xr(daData, base_norm_horiz='time', graficas=False): #recibe da de daTodos. Versión con numpy
    ...
    daNorm = xr.apply_ufunc(normaliza_t_aux,
                            daData,
                            eje_x,
                            base_norm_horiz,
                            input_core_dims=[['time'], ['time'], []],  # list with one entry per arg
                            output_core_dims=[['AngBielaInRepe']],  # returned data has one dimension
                            exclude_dims=set(('AngBielaInRepe',)),  # dimensions allowed to change size. Must be a set!
                            vectorize=True,  # loop over non-core dims
                            ).assign_coords(dict(AngBielaInRepe=np.arange(361), # hay que meter esto a mano. Coords en grados 
                                                  AngBielaInRepe_rad = ('AngBielaInRepe', np.deg2rad(np.arange(361))) #Coords en radianes
                                                  )
                                            )
...
# =============================================================================
def calcula_descrip_antropo(dagb, daDataNorm):
    ...
    HJC = daDataNorm.sel(ID=dagb.ID.data, side=['L','R'], n_var='HJC', AngBielaInRepe=180)
    KJC = daDataNorm.sel(ID=dagb.ID.data, side=['L','R'], n_var='KJC', AngBielaInRepe=180)
    AJC = daDataNorm.sel(ID=dagb.ID.data, side=['L','R'], n_var='AJC', AngBielaInRepe=180)
    muslo = np.sqrt((HJC.sel(axis='x') - KJC.sel(axis='x'))**2 + (HJC.sel(axis='y') - KJC.sel(axis='y'))**2 + (HJC.sel(axis='z') - KJC.sel(axis='z'))**2).mean(dim='phase')
    pierna = np.sqrt((KJC.sel(axis='x') - AJC.sel(axis='x'))**2 + (KJC.sel(axis='y') - AJC.sel(axis='y'))**2 + (KJC.sel(axis='z') - AJC.sel(axis='z'))**2).mean(dim='phase')
    ...
    dfAntropo = (xr.concat([muslo, pierna], pd.Index(['muslo','pierna'], name='segmento'))
                 # .transpose('ID', 'segmento', 'side')
                  .to_dataframe()
                  .reset_index()
                  .drop(columns=['axis'])
                  .reindex(columns=['ID', 'segmento', 'side', 'Cinem'])
                )
    
    #Calcula ancho caderas y pedales y lo añade
    ancho_caderas = abs(dagb.sel(n_var='HJC', side='R', axis='x').data - dagb.sel(n_var='HJC', side='L', axis='x').data).mean()
    ...
    long_cluster_pedales = 10 #TODO: ¿HACERLO CON MARCADOR METAS?
    ancho_pedales = -long_cluster_pedales*2 + abs(dagb.sel(n_var='PosPedal', side='R', axis='x') - dagb.sel(n_var='PosPedal', side='L', axis='x')).mean().data / 10
    #(dagb.sel(n_var='PosPedal', side='R', axis='x') - dagb.sel(n_var='PosPedal', side='L', axis='x')).plot.line(x='time')
    
    dfAntropo.loc[dfAntropo.shape[0]] = [dfAntropo.iloc[0,0], 'ancho_caderas', 'LR', ancho_caderas]
    dfAntropo.loc[dfAntropo.shape[0]] = [dfAntropo.iloc[0,0], 'ancho_pedales', 'LR', ancho_pedales]
    
    #Ángulos en posiciones de biela
    dfNorm = daDataNorm.sel(ID=dagb.ID).to_dataframe(name='value').dropna().reset_index()   
    ...
    resumen_discreto = dfNorm.query('AngBielaInRepe==[0, 90, 180]').query('n_var==["AngArtHip", "AngArtKnee", "AngArtAnkle", "AngSegPELVIS"]').groupby(['ID', 'n_var', 'AngBielaInRepe', 'axis', 'side'])['value'].agg(['mean', 'std']).reset_index()
    ...
    L = daDataNorm.sel(ID=dagb.ID.data, side='L', n_var=['AngArtHip', 'AngArtKnee', 'AngArtAnkle'], axis='x').mean(dim='phase')
    R = daDataNorm.sel(ID=dagb.ID.data, side='R', n_var=['AngArtHip', 'AngArtKnee', 'AngArtAnkle'], axis='x').mean(dim='phase')
    ...
    dfCompLados = (rmse.to_dataframe().reset_index()
                  .reset_index()
                  .drop(columns=['axis'])
                  .reindex(columns=['ID', 'n_var', 'Cinem'])
...
def RealizaGraficas_cinem(daGraf, repes=[0,10],
                          nomvars = ['AngArtKnee', 'AngArtHip', 'AngArtAnkle'],
                          ejes=['x', 'y'],
                          tipo_graf=['lados_lin', 'lados_circ', 'coordinacion', 'planoXY'],
                          compara_lados_graf=False,
                          ensemble_avg=True,
                          carpeta_guardar=False): #por mejorar: que se puedan descartar repeticiones intermedias
    import itertools    
    """
    Nomenclatura:
    ...
    if repes==None: #si no se indica nada, muestra todas las repeticiones
        ...
        if repes[1] > numreps:
            repes[1] = numreps  
    rango=np.arange(repes[0],repes[1]).tolist() #rango de repeticiones para todas las gráficas
    ...
    def guarda_grafica(nom, carpeta_guardar):
        if not carpeta_guardar:
            return
        #print(carpeta_guardar)
        #print(formatoImagenes)
        ruta_fig = carpeta_output.joinpath(dfGraf['ID'].iloc[0] + nom).with_suffix(formatoImagenes)
    ...
    with sns.plotting_context(context='paper', rc=rc):
        ...
        if 'lados_lin' in tipo_graf:        
            for nomvar, eje in itertools.product(nomvars, ejes):
                # print(nomvar,eje)
                titulo = u'{0:s} {1:s}'.format(nfa.traduce_variables(nomvar), nfa.traduce_variables(eje))
                ...
                if ensemble_avg=='completo':
                    sns.lineplot(data=dfGraf.query('n_var==@nomvar & axis==@eje'), x='AngBielaInRepe', y='value', hue='side', estimator=None, errorbar=ci, units='phase', lw=0.25, palette=['r', 'lime'], alpha=0.3, zorder=2, ax=ax) #'darkgrey', 'darkgrey'
                g = sns.lineplot(data=dfGraf.query('n_var==@nomvar & axis==@eje'), x='AngBielaInRepe', y='value', hue='side', estimator=estim, errorbar=ci, units=unit, lw=1, palette=['r', 'lime'], alpha=alpha, zorder=3, ax=ax)
                #Ajusta formato gráfica
                #g.figure.suptitle('{0:s} ({1:s})'.format(dfLateral['ID'].iloc[0], nomvar))
                
                if nomvar=='AngArtKnee':
                    ax.axhspan(25, 35, alpha=0.3, color='green') #crea un rectángulo horizontal delimitado a mano
                    ax.text(5, 31, 'Rango recomendado', c='green', alpha=0.6, fontsize=5, transform=ax.transData)
                    ax.text(5, 27, 'máxima extensión', c='green', alpha=0.6, fontsize=5, transform=ax.transData)
                
                g.set_title(titulo) #'{0:s} ({1:s})'.format(dfLateral['ID'].iloc[0], nomvar))
                g.set(xlim=(0,360), xticks=np.linspace(0, 360, 5))
                
                #Excepción para velocidad angular biela
                if nomvar=='vAngBiela':
                    ylabel=u'Velocidad angular (grados/s)'
                    media_vAng = dfGraf.query('n_var==@nomvar & axis==@eje & side=="L"')['value'].mean()
                    ax.axhline(y=media_vAng, ls='-.', lw=0.5, dash_capstyle='round', c='k', alpha=0.5)
                    ax.text(5, media_vAng, 'Frecuencia media: {0:.1f} rpm'.format(media_vAng*60/360), va='bottom', c='k', alpha=0.6, fontsize=5, transform=ax.transData)
                    
                    
                else:
                    ylabel=u'Ángulo (grados)'
                g.set(xlabel=u'Ángulo de biela (grados)', ylabel=ylabel)#($^\circ$)
                
                g.xaxis.grid(True, linestyle='dashed', dashes=(5, 5), dash_capstyle='round', zorder=1)
                ...
                g.legend(handles=custom_lines, loc='best')#bbox_to_anchor=(.95,.95))#, bbox_transform=ax_ang.transAxes)
            
                            
                plt.tight_layout()
                
                guarda_grafica('_A_' + nfa.traduce_variables(nomvar) + '_' + eje, carpeta_guardar)
        ...
        if 'lados_circ' in tipo_graf:
            for nomvar, eje in itertools.product(nomvars, ejes):
                ...
                fig, ax = plt.subplots(figsize=(3,3), subplot_kw=dict(projection='polar'), dpi=300)
                if ensemble_avg=='completo':
                    sns.lineplot(data=dfGraf.query('n_var == @nomvar & axis ==@eje'), x='AngBielaInRepe_rad', y='value', hue='side', estimator=None, errorbar=95, units='phase', lw=0.25, palette=['r', 'lime'], alpha=0.3, ax=ax) #'darkgrey', 'darkgrey'
                g = sns.lineplot(data=dfGraf.query('n_var == @nomvar & axis ==@eje'), x='AngBielaInRepe_rad', y='value', hue='side', estimator=estim, errorbar=ci, units=unit, lw=1, palette=['r', 'lime'], alpha=alpha, ax=ax)
                ...
                g.legend(handles=custom_lines, bbox_to_anchor=(.0,0.95))#, bbox_transform=ax_ang.transAxes)
            
                plt.tight_layout()
                
                guarda_grafica('_AP_' + nfa.traduce_variables(nomvar) + '_' + eje, carpeta_guardar)
        ...
        if 'coordinacion' in tipo_graf:
            for eje in ejes:
                for parnomvar in itertools.combinations(nomvars,2): 
                    ...
                    df = dfGraf.query('n_var==@parnomvar[0] & axis ==@eje')[['ID', 'repe', 'side', 'axis', 'AngBielaInRepe']].reset_index(drop=True)
                    df[parnomvar[0]] = dfGraf.query('n_var==@parnomvar[0] & axis==@eje')['value'].reset_index(drop=True)
                    df[parnomvar[1]] = dfGraf.query('n_var==@parnomvar[1] & axis==@eje')['value'].reset_index(drop=True)
                    
                    df_ = df
                    y=None
                    if ensemble_avg:
                        y=parnomvar[1]
                        df_ = df.groupby(['side', 'AngBielaInRepe'])[[parnomvar[0], parnomvar[1]]].mean()
                    
                    fig, ax = plt.subplots(figsize=(3,3), dpi=300)
                    #Dibuja líneas (media o repe a repe según ensemble_avg)
                    g = sns.lineplot(data=df_, x=parnomvar[1], y=parnomvar[0], units=unit, hue='side', estimator=None, palette=['r', 'lime'], sort=False, alpha=alpha, ax=ax)
                    
                    #Dibuja la posición en puntos críticos de cada ciclo
                    for posbiela in [0,90,180]:
                        posx=df.query('side=="L" & AngBielaInRepe==@posbiela').loc[:,parnomvar[1]].mean()
                        posy=df.query('side=="L" & AngBielaInRepe==@posbiela').loc[:,parnomvar[0]].mean()
                        ax.plot(posx, posy, c='firebrick', mfc='r', marker='o', ms=13, alpha=0.7, zorder=2)
                        ax.text(posx, posy, str(posbiela), c='firebrick', fontsize=7, fontweight='bold', transform=ax.transData, ha='center', va='center', zorder=3)
                        
                        posx=df.query('side=="R" & AngBielaInRepe==@posbiela').loc[:,parnomvar[1]].mean()
                        posy=df.query('side=="R" & AngBielaInRepe==@posbiela').loc[:,parnomvar[0]].mean()
                        ax.plot(posx, posy, c='limegreen', mfc='lime', marker='o', ms=13, alpha=0.7, zorder=2)
                        ax.text(posx, posy, str(posbiela), c='limegreen', fontsize=7, fontweight='bold', transform=ax.transData, ha='center', va='center', zorder=3)
                    ...
                    if ensemble_avg: 
                        ...
                        #print(nomvar)            
                        err = np.sqrt(df.query('side=="L"').groupby('AngBielaInRepe').std()[parnomvar[0]]**2 + df.query('side=="L" & phase==@rango').groupby('AngBielaInRepe').std()[parnomvar[1]]**2)
                        ...
                        draw_error_band(ax, df_.query('side=="L"')[parnomvar[1]], df_.query('side=="L"')[parnomvar[0]], err=err,
                                        facecolor='r', edgecolor="none", alpha=.2, zorder=0)
                        err = np.sqrt(df.query('side=="R"').groupby('AngBielaInRepe').std()[parnomvar[0]]**2 + df.query('side=="R" & phase==@rango').groupby('AngBielaInRepe').std()[parnomvar[1]]**2)
                        ...
                        draw_error_band(ax, df_.query('side=="R"')[parnomvar[1]], df_.query('side=="R"')[parnomvar[0]], err=err,
                                        facecolor='lime', edgecolor="none", alpha=.2, zorder=0)
                            
                        if ensemble_avg=='completo':
                            #print('dibuja repes sueltas en' + nomvar)
                            for rep in range(dfGraf.phase.max()):
                                ax.plot(df.query('side=="L" & phase==@rep').loc[:,parnomvar[1]], 
                                        df.query('side=="L" & phase==@rep').loc[:,parnomvar[0]], 
                                        c='r',lw=0.25, alpha=0.3, zorder=0)
                                ax.plot(df.query('side=="R" & phase==@rep').loc[:,parnomvar[1]], 
                                        df.query('side=="R" & phase==@rep').loc[:,parnomvar[0]], 
                                        c='limegreen',lw=0.25, alpha=0.3, zorder=0)    
                                        
                    #Ajusta formato gráfica
                    #g.figure.suptitle('{0:s} ({1:s})'.format(dfLateral['ID'].iloc[0], nomvar))
                    g.set_title(u'Coordinación articular, vista {0:s}'.format(nfa.traduce_variables(eje))) #' {0:s}/{1:s} {2:s}'.format(nfa.traduce_variables(parnomvar[0]), nfa.traduce_variables(parnomvar[1]), nfa.traduce_variables(eje))) #'{0:s} ({1:s})'.format(dfLateral['ID'].iloc[0], nomvar))
                    
                    g.set(xlabel=f'{nfa.traduce_variables(parnomvar[1])} (grados)', ylabel=f'{nfa.traduce_variables(parnomvar[0])} (grados)')
                    ...
                    guarda_grafica('_AA_' + nfa.traduce_variables(parnomvar[0]) +'_'+ nfa.traduce_variables(parnomvar[1]) + '_' + eje, carpeta_guardar)
        ...
        if 'indiv_lin' in tipo_graf:
            for nomvar, eje in itertools.product(nomvars, ejes):
                ...
                if nomvar=='AngSegPELVIS' and eje=='y':
                    titulo = u'Basculación lateral pelvis'
                else:            
                    titulo = u'{0:s} {1:s}'.format(nfa.traduce_variables(nomvar), nfa.traduce_variables(eje))
                    
                fig, ax = plt.subplots(figsize=(4,3), dpi=300)
                if ensemble_avg=='completo':
                    sns.lineplot(data=dfGraf.query('n_var == @nomvar & axis ==@eje'), x='AngBielaInRepe', y='value', estimator=None, errorbar=95, units='phase', lw=0.25, color='orange', alpha=0.3, zorder=1, ax=ax) #'lightgrey'
                g = sns.lineplot(data=dfGraf.query('n_var == @nomvar & axis ==@eje'), x='AngBielaInRepe', y='value', estimator=estim, errorbar=ci, units=unit, lw=1, color='orange', alpha=alpha, zorder=3, ax=ax)
                
                #Ajusta formato gráfica
                #g.figure.suptitle('{0:s} ({1:s})'.format(dfLateral['ID'].iloc[0], nomvar))
                g.set_title(titulo) #'{0:s} ({1:s})'.format(dfLateral['ID'].iloc[0], nomvar))
                g.set(xlim=(0,360), xticks=np.linspace(0, 360, 5))
                g.set(xlabel=u'Ángulo de biela (grados)', ylabel=u'Ángulo (grados)')#($^\circ$)
                
                g.xaxis.grid(True, linestyle='dashed', dashes=(5, 5), dash_capstyle='round', zorder=1)
                
                media = dfGraf.query('n_var == @nomvar & axis ==@eje')['value'].mean()
                ax.axhline(y=media, ls='-.', lw=0.5, dash_capstyle='round', c='orange', alpha=0.5)
                ax.text(3, media, 'Posición central', va='bottom', c='orange', alpha=0.6, fontsize=6, transform=ax.transData)
                                    
                plt.tight_layout()
                
                guarda_grafica('_I_' + nfa.traduce_variables(nomvar) +'_' + eje, carpeta_guardar)
        ...
        if 'indiv_circ' in tipo_graf:
            for nomvar, eje in itertools.product(nomvars, ejes):
                ...
                fig, ax = plt.subplots(figsize=(3,3), subplot_kw=dict(projection='polar'), dpi=300)
                if ensemble_avg=='completo':
                    sns.lineplot(data=dfGraf.query('n_var == @nomvar & axis ==@eje'), x='AngBielaInRepe_rad', y='value', estimator=None, errorbar=95, units='phase', lw=0.25, color='orange', alpha=0.3, ax=ax) #'lightgrey'
                g = sns.lineplot(data=dfGraf.query('n_var == @nomvar & axis ==@eje'), x='AngBielaInRepe_rad', y='value', estimator=estim, errorbar=ci, units=unit, lw=1, color='orange', alpha=alpha, ax=ax)
                ...
                guarda_grafica('_IP_' + nfa.traduce_variables(nomvar) +'_' + eje, carpeta_guardar)
                ...
                for parejes in itertools.combinations(ejes,2):
                    ...
                    if parejes[0]=='x':
                        try: #solución provisional
                            ...
                            offsetX = ((daGraf.sel(n_var='PosPedal', axis='x', side='L') + daGraf.sel(n_var='PosPedal', axis='x', side='R')) / 2).mean(dim=['AngBielaInRepe', 'phase']).data / 10 #lo pasa a cm
                        
                        df[var1]-=offsetX #var1.query('side=="Der"').min()['var1'] #solo para ajustar el cero del eje x
                    ...
                    if ensemble_avg:
                        y=var2
                        df_ = df.groupby(['side', 'AngBielaInRepe'])[[var1, var2]].mean()
                    
                    
                    fig, ax = plt.subplots(figsize=(3,3), dpi=300)
                    #Dibuja líneas (media o repe a repe según ensemble_avg)
                    g = sns.lineplot(data=df_, x=var1, y=var2, units=unit, hue='side', estimator=None, palette=['r', 'lime'], sort=False, alpha=alpha, ax=ax)
                    
                    #Dibuja la posición en puntos críticos de cada ciclo
                    for posbiela in [0,90,180]:
                        posx=df.query('side=="L" & AngBielaInRepe==@posbiela').loc[:,var1].mean()
                        posy=df.query('side=="L" & AngBielaInRepe==@posbiela').loc[:,var2].mean()
                        ax.plot(posx, posy, c='firebrick', mfc='r', marker='o', ms=13, alpha=0.7, zorder=2)
                        ax.text(posx, posy, str(posbiela), c='firebrick', fontsize=7, fontweight='bold', transform=ax.transData, horizontalalignment='center', verticalalignment='center', zorder=3)
                        
                        posx=df.query('side=="R" & AngBielaInRepe==@posbiela').loc[:,var1].mean()
                        posy=df.query('side=="R" & AngBielaInRepe==@posbiela').loc[:,var2].mean()
                        ax.plot(posx, posy, c='limegreen', mfc='lime', marker='o', ms=13, alpha=0.7, zorder=2)
                        ax.text(posx, posy, str(posbiela), c='limegreen', fontsize=7, fontweight='bold', transform=ax.transData, horizontalalignment='center', verticalalignment='center', zorder=3)
                        
                    #Línea eje central
                    ax.axvline(x=0, c='grey', ls='--', dashes=(5, 5), dash_capstyle='round', alpha=0.5)
                    
                    #sombrea la zona de desviación típica si ensemble_avg no es False
                    if ensemble_avg: 
                        #for nomvar, ax in g.axes_dict.items():
                        #print(nomvar)            
                        err = np.sqrt(df.query('side=="L"').groupby('AngBielaInRepe')[var1].std()**2 + df.query('side=="L" & phase==@rango').groupby('AngBielaInRepe')[var2].std()**2)
                        #err = (df.query('comparacion==@nomvar & side=="Izq" & repe==@rango').groupby('AngBielaInRepe').std().var1 + df.query('comparacion==@nomvar & side=="Izq" & repe==@rango').groupby('AngBielaInRepe').std().var2)/2
                        draw_error_band(ax, df_.query('side=="L"')[var1], df_.query('side=="L"')[var2], err=err,
                                        facecolor='r', edgecolor="none", alpha=.2, zorder=0)
                        err = np.sqrt(df.query('side=="R"').groupby('AngBielaInRepe')[var1].std()**2 + df.query('side=="R" & phase==@rango').groupby('AngBielaInRepe')[var2].std()**2)
                        #err = (df.query('comparacion==@nomvar & side=="Der" & repe==@rango').groupby('AngBielaInRepe').std().var1 + df.query('comparacion==@nomvar & side=="Der" & repe==@rango').groupby('AngBielaInRepe').std().var2)/2
                        draw_error_band(ax, df_.query('side=="R"')[var1], df_.query('side=="R"')[var2], err=err,
                                        facecolor='lime', edgecolor="none", alpha=.2, zorder=0)
                            
                        if ensemble_avg=='completo':
                            #print('dibuja repes sueltas en' + nomvar)
                            for rep in range(dfGraf.repe.max()):
                                ax.plot(df.query('side=="L" & phase==@rep').loc[:,var1], 
                                        df.query('side=="L" & phase==@rep').loc[:,var2], 
                                        c='r',lw=0.25, alpha=0.3, zorder=0)
                                ax.plot(df.query('side=="R" & phase==@rep').loc[:,var1], 
                    ...
                    if nomvar=='KJC' and parejes==('x','z'):
                        try: #solución provisional para cuando se llama individualmente desde Nexus o desde el tratamiento global
                            eje_medio = -daGraf.isel(ID=0).sel(n_var=['HJC', 'AJC'], axis='x').mean(dim=['AngBielaInRepe', 'phase'])
                        except:
                            eje_medio = -daGraf.sel(n_var=['HJC', 'AJC'], axis='x').mean(dim=['AngBielaInRepe', 'phase'])
                        #Lado L
                        ax.axvspan(eje_medio.sel(n_var='HJC', side='L')+offsetX, eje_medio.sel(n_var='AJC', side='L')+offsetX, alpha=0.3, color='green') #crea un rectángulo horizontal delimitado a mano
                        ax.text(eje_medio.sel(n_var='AJC', side='L')+offsetX, ax.get_ylim()[1], 'Eje tobillo', ha='left', va='top', c='green', alpha=0.6, fontsize=6, rotation='vertical', transform=ax.transData, zorder=5)
                        ...
                        ax.text(eje_medio.sel(n_var='HJC', side='L')+offsetX, ax.get_ylim()[1], 'Eje cadera', ha='right', va='top', c='green', alpha=0.6, fontsize=6, rotation='vertical', transform=ax.transData, zorder=5)
                        
                        
                        #Lado R
                        ax.axvspan(eje_medio.sel(n_var='HJC', side='R')+offsetX, eje_medio.sel(n_var='AJC', side='R')+offsetX, alpha=0.3, color='green') #crea un rectángulo horizontal delimitado a mano
                        ax.text(eje_medio.sel(n_var='AJC', side='R')+offsetX, ax.get_ylim()[1], 'Eje tobillo', ha='right', va='top', c='green', alpha=0.6, fontsize=6, rotation='vertical', transform=ax.transData, zorder=5)
                        ax.text(eje_medio.sel(n_var='HJC', side='R')+offsetX, ax.get_ylim()[1], 'Eje cadera', ha='left', va='top', c='green', alpha=0.6, fontsize=6, rotation='vertical', transform=ax.transData, zorder=5)
                        
                    
                    #Ajusta formato gráfica
                    #g.figure.suptitle('{0:s} ({1:s})'.format(dfLateral['ID'].iloc[0], nomvar))
                    g.set_title(u'Vista {0:s} {1:s}'.format(nfa.traduce_variables([parejes[0], parejes[1]]), nfa.traduce_variables(nomvar))) #'{0:s} ({1:s})'.format(dfLateral['ID'].iloc[0], nomvar))
                    ...
                    guarda_grafica('_P_' +nfa.traduce_variables(nomvar) + '_'+ nfa.traduce_variables([parejes[0], parejes[1]]), carpeta_guardar)
...
def RealizaGraficas_triples_cinem(daGraf,
                                  repes=[0,10],
                                  tipo_graf=['lados_lin'],
                                  ensemble_avg=False,
                                  carpeta_guardar=False
                                  ): #por mejorar: que se puedan descartar repeticiones intermedias
    import itertools    
    """
    Nomenclatura:
        '_A_': Gráfica Ángulo de articulación en escala grados biela.
    ...
    with sns.plotting_context(context='paper', rc=rc):
        #Gráficas ÁNGULO / BIELA repetición a repetición con variables por separado
        if 'lados_lin' in tipo_graf:                
            g = sns.relplot(data=dfGraf, x='AngBielaInRepe', y='value', col='n_var', hue='side', estimator=estim, errorbar=ci, units=unit, 
                            lw=1, palette=['r', 'lime'], alpha=alpha, facet_kws={'sharey': False, 'legend_out':False, 'despine':False}, solid_capstyle='round', kind='line', 
                            legend=True, aspect=1.2, height=3, zorder=2)
            if ensemble_avg=='completo':
                #Dibuja todas las repeticiones
                g.map_dataframe(sns.lineplot, x='AngBielaInRepe', y='value', hue='side', estimator=None, errorbar=95, units='phase', lw=0.25, palette=['r', 'lime'], alpha=0.3)
                        
            (g.set_axis_labels('Ángulo de biela (grados)', 'Ángulo (grados)')#, size='large') #pone etiquetas en ejes (las del x todas iguales)
              .set_titles(col_template='{col_name}')#, fontweight='bold') #títulos de cada gráfica
            ...
            sns.move_legend(g, loc='upper right', bbox_to_anchor=(0.99, 1.02),  title='', frameon=True)
            ...
            guarda_grafica(nom='_A_Triple_' + dfGraf.axis.unique()[0])
        ...
        if 'lados_circ' in tipo_graf:            
            ...
            g = sns.FacetGrid(dfGraf, col='n_var', hue='side',  
                      subplot_kws=dict(projection='polar'), height=4, legend_out=False,
                      sharex=False, sharey=False, despine=False, palette=['r', 'lime'])
            
            if ensemble_avg=='completo':
                #Dibuja todas las repeticiones
                g.map_dataframe(sns.lineplot, x='AngBielaInRepe_rad', y='value', estimator=None, errorbar=None, units='phase', lw=0.25, palette=['r', 'lime'], alpha=0.3)
            #Dibuja gráfica principal (después que línea a línea para que mantenga las características de la leyenda)
            g.map_dataframe(sns.lineplot, x='AngBielaInRepe_rad', y='value', estimator=estim, errorbar=ci, units=unit, lw=1,alpha=alpha)
                        
            g.add_legend()
            (g.set_axis_labels('', '') #pone etiquetas en ejes (las del x todas iguales)
              .set_titles(col_template='{col_name}')#, fontweight='bold') #títulos de cada gráfica
            ...
            #Ajusta leyenda
            sns.move_legend(g, loc='upper right', bbox_to_anchor=(0.98, .95),  title='', frameon=True)
            """#En la leyenda sustituye por en español
            custom_lines = [Line2D([0], [0], color='r', lw=2, solid_capstyle='round', label='Izq'),
                            Line2D([0], [0], color='lime', lw=2, solid_capstyle='round', label='Der'),
                           ]
            ...
            guarda_grafica(nom='_AP_Triple_' + dfGraf.axis.unique()[0])
        ...
        if 'coordinacion' in tipo_graf:
            ...
            df=[]
            for comp, parnomvar in enumerate(itertools.combinations(dfGraf.n_var.unique(),2)): 
                #print(comp,parnomvar)
                df.append(pd.concat([dfGraf.rename(columns = {'value':'var1'}).query('n_var==@parnomvar[0]').reset_index(drop=True), 
                                     dfGraf.rename(columns = {'value':'var2'}).query('n_var==@parnomvar[1]')['var2'].reset_index(drop=True)], axis=1).assign(**{'comparacion':parnomvar[0]+'/'+parnomvar[1]}))
            df = pd.concat(df).sort_values(by=['n_var', 'side']) #ordena para que mantenga el orden tras otras operaciones
            ...
            if ensemble_avg:
                y='var2'
                df_ = df.groupby(['comparacion', 'side', 'AngBielaInRepe'])[['var1', 'var2']].mean()
            
            #Dibuja líneas (media o repe a repe según ensemble_avg)
            g = sns.relplot(data=df_, x='var2', y='var1', col='comparacion', hue='side', #estimator=estim, ci=ci, units=unit, 
                            lw=1, palette=['lime', 'r'], alpha=alpha, facet_kws={'sharex': False, 'sharey': False, 'legend_out':False, 'despine':False}, sort=False, kind='line', 
            ...
            #                 legend=True, height=4)#, zorder=1)
            (g.set_titles(col_template='{col_name}')#, fontweight='bold') #títulos de cada gráfica
              #.set_axis_labels('Ángulo de biela (grados)', 'Ángulo (grados)', size='large') #pone etiquetas en ejes (las del x todas iguales)
              #.tight_layout(w_pad=2) #separación entre gráficas          
            )
            
            sns.move_legend(g, loc='upper right', bbox_to_anchor=(0.99, 0.915), title='', frameon=True)   
            
            #Ajusta gráfica a gráfica
            for nomvar, ax in g.axes_dict.items():
                #print(nomvar)            
                ax.set_xlabel(f'{nomvar.split("/")[1]} (grados)')#, size='large') #($^\circ$)
                ...
                #Dibuja inicios de cada ciclo
                for posbiela in [0,90,180]:
                    posx=df.query('side=="Izq" & comparacion==@nomvar & AngBielaInRepe==@posbiela').loc[:,'var2'].mean()
                    posy=df.query('side=="Izq" & comparacion==@nomvar & AngBielaInRepe==@posbiela').loc[:,'var1'].mean()
                    ax.plot(posx, posy, c='firebrick', mfc='r', marker='o', ms=14, alpha=0.7, zorder=2)
                    ax.text(posx, posy, str(posbiela), c='firebrick', fontsize=8, fontweight='bold', transform=ax.transData, horizontalalignment='center', verticalalignment='center', zorder=3)
                                        
                    posx=df.query('side=="Der" & comparacion==@nomvar & AngBielaInRepe==@posbiela').loc[:,'var2'].mean()
                    posy=df.query('side=="Der" & comparacion==@nomvar & AngBielaInRepe==@posbiela').loc[:,'var1'].mean()
                    ax.plot(posx, posy, c='limegreen', mfc='lime', marker='o', ms=14, alpha=0.7, zorder=2)
                    ax.text(posx, posy, str(posbiela), c='limegreen', fontsize=8, fontweight='bold', transform=ax.transData, horizontalalignment='center', verticalalignment='center', zorder=3)
                ...
                if ensemble_avg: 
                    #for nomvar, ax in g.axes_dict.items():
                    #print(nomvar)            
                    err = np.sqrt(df.query('comparacion==@nomvar & side=="Izq"')[['AngBielaInRepe','var1']].groupby('AngBielaInRepe').std()**2 + df.query('comparacion==@nomvar & side=="Izq" & phase==@rango')[['AngBielaInRepe','var2']].groupby('AngBielaInRepe').std()**2)
                    #err = (df.query('comparacion==@nomvar & side=="Izq" & repe==@rango').groupby('AngBielaInRepe').std().var1 + df.query('comparacion==@nomvar & side=="Izq" & repe==@rango').groupby('AngBielaInRepe').std().var2)/2
                    draw_error_band(ax, df_.query('comparacion==@nomvar & side=="Izq"').var2, df_.query('comparacion==@nomvar & side=="Izq"').var1, err=err,
                                    facecolor='r', edgecolor="none", alpha=.2, zorder=0)
                    err = np.sqrt(df.query('comparacion==@nomvar & side=="Der"').groupby('AngBielaInRepe').std().var1**2+ df.query('comparacion==@nomvar & side=="Der" & phase==@rango').groupby('AngBielaInRepe').std().var2**2)
                    #err = (df.query('comparacion==@nomvar & side=="Der" & repe==@rango').groupby('AngBielaInRepe').std().var1 + df.query('comparacion==@nomvar & side=="Der" & repe==@rango').groupby('AngBielaInRepe').std().var2)/2
                    draw_error_band(ax, df_.query('comparacion==@nomvar & side=="Der"').var2, df_.query('comparacion==@nomvar & side=="Der"').var1, err=err,
                                    facecolor='lime', edgecolor="none", alpha=.2, zorder=0)
                    ...
                    if ensemble_avg=='completo':
                        #print('dibuja repes sueltas en' + nomvar)
                        for rep in range(dfGraf.repe.max()):
                            ax.plot(df.query('side=="Izq" & comparacion==@nomvar & phase==@rep').loc[:,'var2'], 
                                    df.query('side=="Izq" & comparacion==@nomvar & phase==@rep').loc[:,'var1'], 
                                    c='r',lw=0.25, alpha=0.3, zorder=0)
                            ax.plot(df.query('side=="Der" & comparacion==@nomvar & phase==@rep').loc[:,'var2'], 
                                    df.query('side=="Der" & comparacion==@nomvar & phase==@rep').loc[:,'var1'], 
                                    c='limegreen',lw=0.25, alpha=0.3, zorder=0)    
            ...
            guarda_grafica(nom='_AA_Triple_' + dfGraf.axis.unique()[0])
        ...
        if 'planoXY' in tipo_graf:
            #Crea un dataframe con las comparaciones pareadas y con columna con nº comparación
            df=[]
            for comp, parejes in enumerate(itertools.combinations(dfGraf.axis.unique(),2)): 
                #print(comp,parejes)
                #var1 = dfGraf.rename(columns = {'value':'var1'}).query('axis==@parejes[0] & n_var != "desconocido"') #desconocido son los datos de pospedal
                var1 = dfGraf.rename(columns = {'value':'var1'}).query('axis==@parejes[0]')
                ...
                if parejes==('x','z'):
                    var1['var1']=-var1['var1']
                    var1['var1']-=var1.query('side=="Der"').min()['var1'] #solo para ajustar el cero del axis x
                #En la visión superior invierte el axis x para que las coordenadas sean positivas
                if parejes==('x','y'):
                    var1['var1']-=var1.iloc[0,var1.columns.get_loc('var1')] #solo para ajustar el cero del axis 
                         
                var2 = dfGraf.rename(columns = {'value':'var2'}).query('axis==@parejes[1]')['var2']
                
                df.append(pd.concat([var1.reset_index(drop=True), 
                                     var2.reset_index(drop=True)], axis=1).assign(**{'comparacion':parejes[0]+'/'+parejes[1]}))
            df = pd.concat(df).sort_values(by=['n_var', 'side']) #ordena para que mantenga el orden tras otras operaciones
            df['comparacion'] = df['comparacion'].replace({'x/y':'Vista cenital', 'x/z':'Vista frontal', 'y/z':'Vista lateral'})
            ...
            #Dibuja líneas (media o repe a repe según ensemble_avg)
            g = sns.relplot(data=df_, x='var1', y='var2', col='comparacion', hue='side', #estimator=estim, ci=ci, units=unit, 
                            lw=1, palette=['lime', 'r'], alpha=alpha, facet_kws={'sharex': False, 'sharey': False, 'legend_out':False, 'despine':False}, sort=False, kind='line', 
                            legend=True, height=4, zorder=1)
            ...
            #                 lw=1, palette=['r', 'lime'], alpha=0.8, facet_kws={'sharex': False, 'sharey': False, 'legend_out':False, 'despine':False}, sort=False, kind='line', 
            #                 legend=True, height=4)#, zorder=1)
            (g.set_titles('{col_name}')#, fontweight='bold') #títulos de cada gráfica
              #.set_axis_labels('Ángulo de biela (grados)', 'Ángulo (grados)', size='large') #pone etiquetas en ejes (las del x todas iguales)
              #.tight_layout(w_pad=2) #separación entre gráficas          
            )
            ...
            for nomvar, ax in g.axes_dict.items():
                ...
                #Dibuja la posición en puntos críticos de cada ciclo                
                for posbiela in [0,90,180]:
                    posx=df.query('side=="Izq" & comparacion==@nomvar & AngBielaInRepe==@posbiela').loc[:,'var1'].mean()
                    posy=df.query('side=="Izq" & comparacion==@nomvar & AngBielaInRepe==@posbiela').loc[:,'var2'].mean()
                    ax.plot(posx, posy, c='firebrick', mfc='r', marker='o', ms=14, alpha=0.7, zorder=2)
                    ax.text(posx, posy, str(posbiela), c='firebrick', fontsize=8, fontweight='bold', transform=ax.transData, horizontalalignment='center', verticalalignment='center', zorder=3)
                                        
                    posx=df.query('side=="Der" & comparacion==@nomvar & AngBielaInRepe==@posbiela').loc[:,'var1'].mean()
                    posy=df.query('side=="Der" & comparacion==@nomvar & AngBielaInRepe==@posbiela').loc[:,'var2'].mean()
                    ax.plot(posx, posy, c='limegreen', mfc='lime', marker='o', ms=14, alpha=0.7, zorder=2)
                    ax.text(posx, posy, str(posbiela), c='limegreen', fontsize=8, fontweight='bold', transform=ax.transData, horizontalalignment='center', verticalalignment='center', zorder=3)
                ...
                if ensemble_avg: 
                    #for nomvar, ax in g.axes_dict.items():
                    #print(nomvar)            
                    err = np.sqrt(df.query('comparacion==@nomvar & side=="Izq"').groupby('AngBielaInRepe').std().var1**2+ df.query('comparacion==@nomvar & side=="Izq" & phase==@rango').groupby('AngBielaInRepe').std().var2**2)
                    #err = (df.query('comparacion==@nomvar & side=="Izq" & repe==@rango').groupby('AngBielaInRepe').std().var1 + df.query('comparacion==@nomvar & side=="Izq" & repe==@rango').groupby('AngBielaInRepe').std().var2)/2
                    draw_error_band(ax, df_.query('comparacion==@nomvar & side=="Izq"').var1, df_.query('comparacion==@nomvar & side=="Izq"').var2, err=err,
                                    facecolor='r', edgecolor="none", alpha=.2, zorder=0)
                    err = np.sqrt(df.query('comparacion==@nomvar & side=="Der"').groupby('AngBielaInRepe').std().var1**2+ df.query('comparacion==@nomvar & side=="Der" & phase==@rango').groupby('AngBielaInRepe').std().var2**2)
                    #err = (df.query('comparacion==@nomvar & side=="Der" & repe==@rango').groupby('AngBielaInRepe').std().var1 + df.query('comparacion==@nomvar & side=="Der" & repe==@rango').groupby('AngBielaInRepe').std().var2)/2
                    draw_error_band(ax, df_.query('comparacion==@nomvar & side=="Der"').var1, df_.query('comparacion==@nomvar & side=="Der"').var2, err=err,
                                    facecolor='lime', edgecolor="none", alpha=.2, zorder=0)
                        
                    if ensemble_avg=='completo':
                        #print('dibuja repes sueltas en' + nomvar)
                        for rep in range(dfGraf.repe.max()):
                            ax.plot(df.query('side=="Izq" & comparacion==@nomvar & phase==@rep').loc[:,'var1'], 
                                    df.query('side=="Izq" & comparacion==@nomvar & phase==@rep').loc[:,'var2'], 
                                    c='r',lw=0.25, alpha=0.3, zorder=0)
                            ax.plot(df.query('side=="Der" & comparacion==@nomvar & phase==@rep').loc[:,'var1'], 
                                    df.query('side=="Der" & comparacion==@nomvar & phase==@rep').loc[:,'var2'], 
                                    c='limegreen',lw=0.25, alpha=0.3, zorder=0)    
            ...
            guarda_grafica(nom='_PP_'+dfGraf['n_var'][0]+'_Triple')
        ...
        if 'planoXYMultiple' in tipo_graf:
            ...
            for comp, parejes in enumerate(itertools.combinations(dfGraf.axis.unique(),2)): 
                ...
                dfm.append(pd.concat([var1.reset_index(drop=True), 
                                     var2.reset_index(drop=True)], axis=1).assign(**{'comparacion':parejes[0]+'/'+parejes[1]}))
            dfm = pd.concat(dfm).sort_values(by=['n_var', 'side']) #ordena para que mantenga el orden tras otras operaciones
            dfm['comparacion'] = dfm['comparacion'].replace({'x/y':'Vista cenital', 'x/z':'Vista frontal', 'y/z':'Vista lateral'})
            #df['axis'] = df['comparacion'].replace({'x/y':'Superior', 'x/z':'frontal', 'y/z':'lateral'})
            ...
            def draw_muscle(x,y,**kwargs):
                ...
                for n, gb in kwargs['data'].groupby('n_var'):
                    plt.plot(gb['var1'],
                             gb['var2'], c=kwargs['color'], lw=kwargs['lw'])
                    
                                
            
            g = sns.FacetGrid(dfm, col='comparacion', hue='side',# col_order=['Izq', 'Der'],
                       legend_out=False, sharex=False, sharey=False,
                       height=4, despine=False, palette=['r', 'lime'])
            ...
            for nomvar, ax in g.axes_dict.items():
                ...
                if nomvar=='Vista cenital':
                    xlabel='mediolateral'
                    ylabel= 'anteroposterior'
                    ax.axvline(x=0, c='grey', ls='--', dashes=(5, 5), lw=1, dash_capstyle='round', alpha=0.5)
                elif nomvar=='Vista frontal':
                    xlabel='mediolateral'
                    ylabel= 'vertical'
                    ax.axvline(x=0, c='grey', ls='--', dashes=(5, 5), lw=1, dash_capstyle='round', alpha=0.5)
        ...
        if '3D_lin' in tipo_graf:                
            dfGraf['axis'].replace({'x':'Vista '+nfa.traduce_variables('x'), 'y':'Vista '+nfa.traduce_variables('y'), 'z':'Vista '+nfa.traduce_variables('z')}, inplace=True) #{'x':u'Basculación ant./post.', 'y':u'Basculación lat.', 'z':u'Rotación'}, inplace=True)

            g = sns.relplot(data=dfGraf, x='AngBielaInRepe', y='value', col='axis', estimator=estim, errorbar=ci, units=unit, 
                            lw=1, color='orange', alpha=alpha, facet_kws={'sharey': False, 'legend_out':False, 'despine':False}, solid_capstyle='round', kind='line', 
                            legend=True, aspect=1.3, height=3, zorder=1)
            ...
            for nomvar, ax in g.axes_dict.items():
                #ax.set_xticks(np.linspace(0, 360, 5))
                #ax.set_xlim(0, 360)
                ax.xaxis.grid(True, linestyle='dashed', dashes=(5, 5), dash_capstyle='round', alpha=0.8, zorder=0)
                #ax.set_xlabel('Ángulo de biela (grados)')
            ...
            if ensemble_avg=='completo':
                ...
                for eje, ax in g.axes_dict.items():
                    for rep in range(dfGraf.repe.max()):
                        ax.plot(range(361), dfGraf.query('axis==@eje & phase==@rep')['value'], c='orange', lw=0.25, alpha=0.3, zorder=0)
                        
    
            #plt.tight_layout()
            
            guarda_grafica(nom='_A3D_'+dfGraf['n_var'][0]+'_Triple')
            #---------------------------------------
...
def RealizaGraficas_EMG(daGraf, repes=[0,10], nomvars=['REC', 'BIC'], tipo_graf=['lados_lin'], otros_datos=None, extra_nom_archivo='', compara_lados_graf=False, ensemble_avg=False, carpeta_guardar=False): #por mejorar: que se puedan descartar repeticiones intermedias
    #import itertools    
    """
    Nomenclatura:
    ...
    with sns.plotting_context(context='paper', rc=rc):
        ...
        if 'lados_lin' in tipo_graf:        
            for nomvar in nomvars:
                ...
                if ensemble_avg=='completo':
                    sns.lineplot(data=dfGraf.query('n_var==@nomvar'), x='AngBielaInRepe', y='value', hue='side', estimator=None, errorbar=95, units='phase', lw=0.25, palette=['r', 'lime'], alpha=0.3, ax=ax) #'darkgrey', 'darkgrey'
                g = sns.lineplot(data=dfGraf.query('n_var==@nomvar'), x='AngBielaInRepe', y='value', hue='side', estimator=estim, errorbar=ci, units=unit, lw=1, palette=['r', 'lime'], alpha=alpha, ax=ax)
                #Ajusta formato gráfica
                #g.figure.suptitle('{0:s} ({1:s})'.format(dfLateral['ID'].iloc[0], nomvar))
                g.set_title(titulo) #'{0:s} ({1:s})'.format(dfLateral['ID'].iloc[0], nomvar))
                g.set(xlim=(0,360), xticks=np.linspace(0, 360, 5))
                g.set(xlabel=u'Ángulo de biela (grados)', ylabel=u'EMG (%MVC)')#($^\circ$)
                ...
                guarda_grafica(nom='_E_' + nfa.traduce_variables(nomvar))
        ...
        if 'lados_circ' in tipo_graf:
            for nomvar in nomvars:
                ...
                fig, ax = plt.subplots(figsize=(3,3), subplot_kw=dict(projection='polar'), dpi=300)
                if ensemble_avg=='completo':
                    sns.lineplot(data=dfGraf.query('n_var==@nomvar'), x='AngBielaInRepe_rad', y='value', hue='side', estimator=None, errorbar=95, units='phase', lw=0.25, palette=['r', 'lime'], alpha=0.3, ax=ax) #'darkgrey', 'darkgrey'
                g = sns.lineplot(data=dfGraf.query('n_var==@nomvar'), x='AngBielaInRepe_rad', y='value', hue='side', estimator=estim, errorbar=ci, units=unit, lw=1, palette=['r', 'lime'], alpha=alpha, ax=ax)
                ...
                g.legend(handles=custom_lines, bbox_to_anchor=(.0,1.2))#, bbox_transform=ax_ang.transAxes)
            
                plt.tight_layout()
                
                guarda_grafica(nom='_EP_' + nfa.traduce_variables(nomvar))
        ...
        if 'coordinacion' in tipo_graf:        
            for parnomvar in nomvars: 
                ...
                #Adapta el dataframe para que tenga las dos variables en columnas
                df = dfGraf.query('n_var==@parnomvar[0]')[['ID', 'phase', 'side', 'AngBielaInRepe']].reset_index(drop=True)
                df[parnomvar[0]] = dfGraf.query('n_var==@parnomvar[0]')['value'].reset_index(drop=True)
                df[parnomvar[1]] = dfGraf.query('n_var==@parnomvar[1]')['value'].reset_index(drop=True)
                ...
                #Dibuja inicios de cada ciclo            
                ax.plot(df.query('side=="Izq" & AngBielaInRepe==0').loc[:,parnomvar[1]].mean(), 
                        df.query('side=="Izq" & AngBielaInRepe==0').loc[:,parnomvar[0]].mean(), 
                        c='firebrick', mfc='r', marker='o', ms=10, alpha=0.5, zorder=2)
                ax.plot(df.query('side=="Der" & AngBielaInRepe==0').loc[:,parnomvar[1]].mean(), 
                        df.query('side=="Der" & AngBielaInRepe==0').loc[:,parnomvar[0]].mean(), 
                        c='limegreen', mfc='lime', marker='o', ms=10, alpha=0.5, zorder=2)
                ...
                if ensemble_avg: 
                    #for nomvar, ax in g.axes_dict.items():
                    #print(nomvar)            
                    err = np.sqrt(df.query('side=="Izq"').groupby('AngBielaInRepe').std()[parnomvar[0]]**2 + df.query('side=="Izq"').groupby('AngBielaInRepe').std()[parnomvar[1]]**2)
                    #err = (df.query('comparacion==@nomvar & side=="Izq" & repe==@rango').groupby('AngBielaInRepe').std().var1 + df.query('comparacion==@nomvar & side=="Izq" & repe==@rango').groupby('AngBielaInRepe').std().var2)/2
                    draw_error_band(ax, df_.query('side=="Izq"')[parnomvar[1]], df_.query('side=="Izq"')[parnomvar[0]], err=err,
                                    facecolor='r', edgecolor="none", alpha=.2, zorder=0)
                    err = np.sqrt(df.query('side=="Der"').groupby('AngBielaInRepe').std()[parnomvar[0]]**2 + df.query('side=="Der"').groupby('AngBielaInRepe').std()[parnomvar[1]]**2)
                    #err = (df.query('comparacion==@nomvar & side=="Der" & repe==@rango').groupby('AngBielaInRepe').std().var1 + df.query('comparacion==@nomvar & side=="Der" & repe==@rango').groupby('AngBielaInRepe').std().var2)/2
                    draw_error_band(ax, df_.query('side=="Der"')[parnomvar[1]], df_.query('side=="Der"')[parnomvar[0]], err=err,
                                    facecolor='lime', edgecolor="none", alpha=.2, zorder=0)
                        
                    if ensemble_avg=='completo':
                        #print('dibuja repes sueltas en' + nomvar)
                        for rep in range(dfGraf.repe.max()):
                            ax.plot(df.query('side=="Izq" & phase==@rep').loc[:,parnomvar[1]], 
                                    df.query('side=="Izq" & phase==@rep').loc[:,parnomvar[0]], 
                                    c='r',lw=0.25, alpha=0.3, zorder=0)
                            ax.plot(df.query('side=="Der" & phase==@rep').loc[:,parnomvar[1]], 
                                    df.query('side=="Der" & phase==@rep').loc[:,parnomvar[0]], 
                                    c='limegreen',lw=0.25, alpha=0.3, zorder=0)    
                ...
                #Ajusta formato gráfica
                #g.figure.suptitle('{0:s} ({1:s})'.format(dfLateral['ID'].iloc[0], nomvar))
                g.set_title(u'Coordinación muscular') #{0:s}/{1:s}'.format(nfa.traduce_variables(parnomvar[0]), nfa.traduce_variables(parnomvar[1]))) #'{0:s} ({1:s})'.format(dfLateral['ID'].iloc[0], nomvar))
                
                g.set(xlabel=f'{nfa.traduce_variables(parnomvar[1])} (%MVC)', ylabel=f'{nfa.traduce_variables(parnomvar[0])} (%MVC)')
                ...
                guarda_grafica(nom='_EE_' + nfa.traduce_variables(parnomvar[0]) +'_'+ nfa.traduce_variables(parnomvar[1]))
        ...
        if 'onset_circ' in tipo_graf:            
            orden_musculos = ['GAS', 'TIB', 'REC', 'VME', 'BIC', 'GLU'] #['GLU', 'BIC', 'VME', 'REC', 'TIB', 'GAS']
            titulo = u'Activación muscular'
            #dfGraf = daGraf.to_dataframe(name='value').dropna().reset_index()
            #dfGraf['side'] = dfGraf['side'].replace({'L':'Izq', 'R':'Der'})
            ...
            ruta = [x for x in ruta if 'PreprocesadoMVC_' +  dfGraf['ID'].unique()[0].split('_')[1]+'.nc' in x.name][0]#.parent
            ...
            #     da_normalizar = procesaEMG(da_normalizar, fc_band=[30, 300], fclow=10, tkeo=True)
                ...
                da_normalizar = da_tkeoMVC.sel(ID=da_tkeoMVC.ID.str.contains(filtro_MVC, case=False))
            if len(da_normalizar.ID)==0:
                print(u'\nNo se han encontrado archivos con el filtro de normalización "{0}"'.format(filtro_MVC))
                sys.exit(0)
            
            #Busca máximo    
            daMVCtkeo_Max = da_normalizar.max(dim=['time','ID'])
            daNomMVCMaxtkeo = da_normalizar.ID[da_normalizar.max(dim='time').argmax(dim='ID')]    
            ...
            da_tkeo = segmenta_ModeloBikefitting_xr_EMG(da_tkeo).sel(n_var=orden_musculos) #se queda solo con los músculos por si hay alguna variable cinemática de segmentar
            #da_tkeo.isel(ID=0).sel(n_var='VME').plot.line(x='time', col='side')
            ...
            nv = 'GLU'
            for l in ['L', 'R']:
                resta = da_tkeo.loc[dict(n_var=nv, side=l, AngBielaInRepe=slice(180,220))].mean(dim=['AngBielaInRepe', 'phase'])
            ...
            df = da_tkeo.assign_coords(side=['Izq', 'Der']).to_dataframe(name='value').dropna().reset_index().groupby(['ID', 'n_var', 'side', 'AngBielaInRepe']).mean().reset_index()
            umbral_onset=1
            #Asigna a cada músculo un valor arbitrario            
            df['value_cat'] = df['n_var'].replace(dict(zip(orden_musculos, range(0, len(orden_musculos))))).astype('int32')
            ...
            g = sns.FacetGrid(df, col='side', hue='n_var', col_order=['Izq', 'Der'], hue_order=orden_musculos,
                      subplot_kws=dict(projection='polar'), height=4, legend_out=False,
                       despine=False)#, palette=['r', 'lime'])
            
            g.map_dataframe(draw_muscle, x='AngBielaInRepe_rad', y='value', lw=10, solid_capstyle='round')
            g.add_legend()
            (g.set_axis_labels('', '') #pone etiquetas en ejes (las del x todas iguales)
              .set_titles(col_template='{col_name}')#, fontweight='bold') #títulos de cada gráfica
              .set(yticks=[])
        ...
        if 'musc_circ' in tipo_graf:
            ...
            g = sns.FacetGrid(df, col='side', hue='n_var', 
                              col_order=['Izq', 'Der'], hue_order=orden_musculos,
                              subplot_kws=dict(projection='polar'), height=4, legend_out=False,
                               despine=False)#, palette=['r', 'lime'])
            
            # Draw a scatterplot onto each axes in the grid
            g.map_dataframe(sns.lineplot, x='AngBielaInRepe_rad', y='value', estimator=estim, errorbar=ci, units=unit, lw=1, solid_capstyle='round')
            g.add_legend()
            (g.set_axis_labels('', '') #pone etiquetas en ejes (las del x todas iguales)
              .set_titles(col_template='{col_name}')#, fontweight='bold') #títulos de cada gráfica
              .set(yticks=[])
        ...
        if 'lados_lin_grupal' in tipo_graf:
            orden_musculos = ['GLU', 'REC', 'VME', 'BIC','GAS', 'TIB', ] #['GLU', 'BIC', 'VME', 'REC', 'TIB', 'GAS']
            titulo = u'Activación muscular'
            #dfGraf = daGraf.to_dataframe(name='value').dropna().reset_index()
            #dfGraf['side'] = dfGraf['side'].replace({'L':'Izq', 'R':'Der'})
            
            g = sns.relplot(data=dfGraf, x='AngBielaInRepe', y='value', col='n_var', col_wrap=3, col_order=orden_musculos, hue='side', estimator=estim, errorbar=ci, units=unit, 
                            lw=1, palette=['r', 'lime'], alpha=alpha, facet_kws={'sharey': True, 'legend_out':False, 'despine':False}, solid_capstyle='round', kind='line', 
                            legend=True, aspect=1.2, height=3, zorder=2)
            ...
            (g.set_titles(col_template='{col_name}')#, fontweight='bold') #títulos de cada gráfica
              .set_axis_labels(xlabel=u'Ángulo de biela (grados)', ylabel=u'EMG (%MVC)') #pone etiquetas en ejes (las del x todas iguales)
              #.set(yticks=[])
              ...
              #.set(ylim=(0, 100))
              .set(xlabel=u'Ángulo de biela (grados)', ylabel=u'EMG (%MVC)')#($^\circ$)
            )
            
            #Ajusta leyenda
            #g.add_legend()
            sns.move_legend(g, loc='upper right', bbox_to_anchor=(1.0, 1.01),  title='', frameon=True) #, bbox_to_anchor=(1.0, 1.01)
            
                        
            for nomvar, ax in g.axes_dict.items():
                ax.set_title(nfa.traduce_variables(nomvar))
                ax.xaxis.grid(True, linestyle='dashed', dash_capstyle='round', alpha=0.8, zorder=1)
                if compara_lados_graf:
                    ti = calcula_spm1d(dfGraf.query('n_var==@nomvar'))
                    #Marca regiones con diferencia L y R
                    plot_clusters(ti, y=5, ax=ax, print_p=False)
            ...
            guarda_grafica(nom='_EG_Grupo' + extra_nom_archivo)
        #------------------------------------------------
        ...
        if 'lados_circ_grupal' in tipo_graf:
            ...
            g = sns.FacetGrid(dfGraf, col='n_var', col_wrap=3, col_order=orden_musculos, hue='side',  
                      subplot_kws=dict(projection='polar'), height=4, legend_out=False,
                      sharex=False, sharey=False, despine=False, palette=['r', 'lime'])
            ...
            guarda_grafica(nom='_EPG_Grupo' + extra_nom_archivo)
        #------------------------------------------------
...
def RealizaGraficas_cinem_completo(daGraf):
    da=daGraf.isel(ID=0)
    RealizaGraficas_triples_cinem(da,
                                  tipo_graf=['lados_lin'],
                                  repes=None,
                                  ensemble_avg='completo'
...
def limpia_MVC(daData):
    from detecta import detect_onset
    
    #daData.sel(n_var=['GAS']).plot.line(x='time', col='side', row='ID')
    
    #Elimina repeticiones con GLU y BIC desconectados
    daData.loc[dict(n_var=['GLU', 'BIC'], ID=(daData.ID.str.contains('-ISO-') & ~daData.ID.isin(['MVC-ISO-{0:02d}'.format(i) for i in range(12,20)])))] = np.nan
    #Elimina repeticiones de maniobra TIB en GAS
    daData.loc[dict(n_var=['GAS'], ID=[daData.ID.data[0].split('MVC')[0] + 'MVC-ISO_{0:02d}'.format(i) for i in range(8,12)])] = np.nan
    #Elimina repeticiones de maniobra GAS, BIC en TIB
    daData.loc[dict(n_var=['TIB'], ID=[daData.ID.data[0].split('MVC')[0] + 'MVC-ISO-{0:02d}'.format(i) for i in list(range(4,8))+list(range(16,20))])] = np.nan
    ...
    def quita_final(data, num, nom_archivo):
        ...
        try:
            ...
            data1[len(data1[~np.isnan(data1)])-num:len(data1[~np.isnan(data1)])+0] = np.zeros(num) #np.nan
    ...
    daData2 = xr.apply_ufunc(quita_final,
                            daData,
                            200, #num. de datos que quita al final
                            daData.ID,
                            input_core_dims=[['time'], [], []],
                            output_core_dims=[['time']],  #datos que devuelve
                            exclude_dims=set(('time', )),  #dimensiones que se permite que cambien (tiene que ser un set)
                            vectorize=True,
...
if __name__ == "__main__":               
    ...
    ruta_trabajo = Path(r'F:\Investigacion\Proyectos\BikeFitting\Bikefitting\EstudioEMG_MVC\Registros\17_Eduardo')
    
    lista_archivos = list(ruta_trabajo.glob('**/*.c3d'))#incluye los que haya en subcarpetas
    lista_archivos = [x for x in lista_archivos if 'MVC-' not in  x.name and 'Estatico' not in x.name]
    lista_archivos.sort()
    
    daDatos = nfa.carga_c3d_generico_xr(lista_archivos[:10], section='Trajectories')  #, nom_vars_cargar=['HJC', 'KJC', 'AJC'])
    ...
    ruta_trabajo = Path(r'F:\Investigacion\Proyectos\BikeFitting\Bikefitting\EstudioEMG_MVC\Registros')
    
    lista_archivos_Din = list(ruta_trabajo.glob('**/*.nc'))#incluye los que haya en subcarpetas
    lista_archivos_Din.sort()
    lista_archivos_Din = [x for x in lista_archivos_Din if 'PreprocesadoDinamicCinem_' in  x.name and not '_Global' in  x.name]
            
    num_cortes = 12
    base_normaliz='time'
    daDin_global_cinem = xr.load_dataarray(lista_archivos_Din[0])
    daDin_global_cinem = daDin_global_cinem.rename({'Archivo':'ID', 'nom_var':'n_var', 'lado':'side', 'eje':'axis'})
    daData_trat = segmenta_ModeloBikefitting_xr_cinem(daDin_global_cinem, num_cortes=num_cortes)
    ...
    RealizaGraficas_cinem(daGraf=daData_trat.isel(ID=0).sel(side=['L', 'R']), tipo_graf=['planoXY'], repes=None, nomvars=['KJC'], ejes=list('yz'), ensemble_avg=bEnsembleAvg, carpeta_guardar=ruta_trabajo/'FigurasGenericas')