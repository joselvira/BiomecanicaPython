...
def integra_completo(daDatos, daEventos):
    ...
    ini = daEventos[0].isel(evento=0).data
    fin = daEventos[0].isel(evento=1).data
    """
    daInt = xr.apply_ufunc(integra, daDatos, daDatos.time, daEventos.isel(evento=0), daEventos.isel(evento=1), daDatos.ID,
...
def RMS(daDatos, ventana):  
    ...
    daRMS = xr.apply_ufunc(rms, daRecortado, 
...
def pasa_dfpl_a_da(dfTodos, merge_2_plats=1, show=False):
    #Transforma df polars a dataarray con todas las variables cargadas
    vars_leidas = dfTodos.select(
                    pl.exclude(['time', 'estudio', 'tipo', 'subtipo', 'ID', 'particip', 'repe']),
                    ).columns
    
    dfpd = (dfTodos.melt(id_vars=['ID', 'time'], value_vars=vars_leidas, variable_name='n_var')
            .to_pandas() 
            )
    daTodos = (dfpd
    ...
    daTodos = asigna_subcategorias_xr(daTodos, dfTodos)
...
def separa_dim_repe_hojaregistro(daDatos, hoja_registro=None):
    ...
    """
    rep0 = xr.concat(rep0, dim='ID')
    rep1 = xr.concat(rep1, dim='ID')
    rep2 = xr.concat(rep2, dim='ID')
    
    daDatos = (xr.concat([rep0, rep1, rep2], dim='repe')
               .assign_coords(repe=[0,1,2])
               )
        
    if np.char.find(daDatos.n_var, 'EMG', start=0, end=None).mean() != -1: #si es EMG        
        daDatos = daDatos.transpose('ID', 'n_var', 'repe', 'time')
    else:
        daDatos = daDatos.transpose('ID', 'n_var', 'repe', 'axis', 'time') #si no es EMG
    return daDatos


#----Separa dimensión plataformas
def separa_dim_plats(da, merge_2_plats):
    #Separa el dataarray en plataformas creando una dimensión
    #Si hay 2 plataformas las agrupa en una
    if merge_2_plats==0:
        plat1 = (da.sel(axis=['Fx', 'Fy', 'Fz'])
                 .assign_coords(axis=['x', 'y', 'z'])
                 )
        plat2 = (da.sel(axis=['Fx_duplicated_0', 'Fy_duplicated_0', 'Fz_duplicated_0'])
                 .assign_coords(axis=['x', 'y', 'z'])
                 )
        ...
        da = (da.sel(axis=['Fx', 'Fy', 'Fz'])
                     .assign_coords(axis=['x', 'y', 'z'])
                     )
    
    elif merge_2_plats==2:
        da = (da.sel(axis=['Fx_duplicated_0', 'Fy_duplicated_0', 'Fz_duplicated_0'])
                     .assign_coords(axis=['x', 'y', 'z'])
                     )
...
def load_merge_bioware_pl(ruta, n_vars_load=None, estudio=None, data_type=None, lin_header=17, merge_2_plats=1, show=False):
    """
    Parameters
    ----------
    ruta_trabajo : TYPE
        DESCRIPTION.
    nom_archivo_preprocesado : TYPE, optional
        DESCRIPTION. The default is None.
    nom_vars_cargar : TYPE, optional
        DESCRIPTION. The default is None.
    tipo_datos:
    ...
    if isinstance(data_type, str): #si se ha definido algún tipo de datos, por defecto es 'float64'
        ...
        dfTodos = dfTodos.select(
                                 #pl.col(['estudio', 'tipo', 'subtipo', 'ID', 'repe']),
    ...
    daTodos = pasa_dfpl_a_da(dfTodos)
           
    
    #TODO: SEGUIR HACIENDO QUE SI CARGA VARIABLES DISTINTAS DE LAS CONVENICONALES LAS PASE A DAARRAY PERO SIN EJES
...
def chequea_igualdad_ini_fin(daDatos, margen=20, ventana=None, retorna=None, show=False):
    ...
    """
    daResta = xr.apply_ufunc(resta_ini_fin_aux, daDatosZ, margen, ventana[0], ventana[1], retorna, daDatos.ID,
                   input_core_dims=[['time'], [], [], [], [], [] ],
                   #output_core_dims=[['time']],
                   exclude_dims=set(('time',)),
                   vectorize=True,
    ...
    if tipo_test=='DJ2P':
        ...
        def detect_aterr1(data):
            ...
            try:                
                ...
                aterr = detect_onset(-data, threshold=-umbral, n_above=int(0.05*daDatos.freq), show=show)[0,0]
                               
            except:
                aterr = 0 #por si no encuentra el criterio
            return float(aterr)
        aterr1 = xr.apply_ufunc(detect_aterr1, daDatos,
                                   input_core_dims=[['time']],
        ...
        aterr2 = detecta_despegue_aterrizaje(daDatos, tipo_test=tipo_test, umbral=umbral).sel(evento='aterrizaje')
            
        daEventos.loc[dict(evento='iniAnalisis')] = xr.where((aterr1 - ventana[0]) >= 0, aterr1 - ventana[0], 0)#.astype(int)
        daEventos.loc[dict(evento='finAnalisis')] = xr.where((aterr2 + ventana[1]) < len(daDatos.time), aterr2 + ventana[1]-1, len(daDatos.time)-1)#.astype(int)
    
    else:
        #Busca despegue y aterrizaje provisionales
        d_a = detecta_despegue_aterrizaje(daDatos, tipo_test=tipo_test, umbral=umbral)
        desp = d_a.sel(evento='despegue').where(d_a.sel(evento='despegue') > 0, np.nan)
        aterr = d_a.sel(evento='aterrizaje').where(d_a.sel(evento='aterrizaje') < len(daDatos.time)-1, np.nan)
        
        daEventos.loc[dict(evento='iniAnalisis')] = xr.where(desp.notnull(), xr.where((desp - ventana[0]) >= 0, desp - ventana[0], 0), np.nan)
        daEventos.loc[dict(evento='finAnalisis')] = xr.where(desp.notnull(), xr.where((aterr + ventana[1]-1) < len(daDatos.time), aterr + ventana[1]-1, len(daDatos.time)-1), np.nan)
        ...
        daEventos.loc[dict(evento='iniAnalisis')] = xr.where((centro_vuelo - ventana[0]) >= 0, centro_vuelo - ventana[0], 0)#.astype(int)
        daEventos.loc[dict(evento='finAnalisis')] = xr.where((centro_vuelo + ventana[1]-1) < len(daDatos.time), centro_vuelo + ventana[1]-1, len(daDatos.time)-1)#.astype(int)
        """
    return daEventos
...
def calcula_peso(daDatos, ventana_peso=None, show=False):
    ...
    if show:
        def dibuja_peso(x,y, **kwargs): #de momento no funciona
            print(x)#kwargs['data'])
            #plt.plot()
        g=daDatos.plot.line(col='ID', col_wrap=3, hue='repe', sharey=False)
            ...
            for i in range(len(ax)): #extrae cada axis (gráfica)     
                ...
                try:
                    ...
                    for j in daDatos.repe:
                        ax[i].axvspan(ventana_peso.sel(ID=idn, repe=j).isel(evento=0)/daDatos.freq, ventana_peso.sel(ID=idn, repe=j).isel(evento=1)/daDatos.freq, alpha=0.2, color=col[j.data-1])
...
def afina_peso(daDatos, daEventos=None, daPeso=None, margen=0.005, tipo_calculo='opt', show=False):
    ...
    def optimiza_peso(data, t, peso, ini, fin, margen, ID):#, rep):
        if np.count_nonzero(~np.isnan(data))==0:
             return np.asarray([np.nan, np.nan])
        try:
            # plt.plot(data)
            ...
            vy=[]
            p=[]
            for pes in range(200, 2000, 50): #calcula velocidad entre pesos extremos cada x datos
                v = integra(data, t, pes) / (pes/9.8)
                #plt.plot(v)
            ...
            #Con polinomio       
            coefs, (resid,_,_,_) = np.polynomial.polynomial.Polynomial.fit(vy, p, deg=8, full=True)
            coefs = coefs.convert().coef #convierte a escala real
            f = np.polynomial.Polynomial(coefs)
            peso = f(0) #peso cuando la velocidad al final es cero
            """
            vy2 = np.arange(-10, 20)
            px = f(vy2) 
            plt.plot(p,vy,'o-')
    ...
    def peso_iter(data, t, peso, ini, fin, margen, ID):#, rep):
        #print(ID, rep)
        if np.count_nonzero(~np.isnan(data))==0:
             return np.asarray([np.nan, np.nan])
...
def detecta_despegue_aterrizaje(daDatos, tipo_test, umbral=10.0, show=False):
    ...
    if tipo_test=='DJ2P':
        def detect_onset_aux(data, coords, **args_func_cortes):
            if np.count_nonzero(~np.isnan(data))==0:
            ...
            if len(ind)<2:
                ...
                if len(ind)<1:
                    print('No se han encontrado dos despegues/aterrizajes en archivo', coords)
                    return np.array([np.nan, np.nan])
            # if tipo_test == 'CMJ':
        ...
        daCorte = xr.apply_ufunc(detect_onset_aux, daDatos, daDatos.ID,
                       input_core_dims=[['time'], []],
        ...
        def detect_onset_aux(data, coords, **args_func_cortes):
            ...
            #Chequea si ha detectado más de un vuelo
            if len(ind) > 1:
                ind = ind[1] if tipo_test=='DJ' else ind[0] #por defecto se queda con el primero o el segundo
                #TODO: mejorar la comprobación cuando detecta más de un vuelo
                # if ind[-1,-1] < int(len(data) * 0.8) or :
                #     ind=ind[-1] #Independientemente del tipo de salto que sea, se queda con el último que encuentre
                # else:
        ...
        daCorte = xr.apply_ufunc(detect_onset_aux, daDatos, daDatos.ID,
                       ...
                       #exclude_dims=set(('time',)),
                       vectorize=True,
                       kwargs=dict(threshold=-umbral, n_above=int(0.15*daDatos.freq), show=show)
                       ).assign_coords(evento=['despegue', 'aterrizaje'])
    #Comprobaciones
    #daDatos.sel(axis='z').isel(time=daCorte.sel(evento='despegue')-1) #despegue cuando ya ha pasado por debajo del umbral
    #daDatos.sel(axis='z').isel(time=daCorte.sel(evento='aterrizaje')-1) #aterrizaje cuando ya ha pasado por debajo del umbral
    return daCorte
...
def detecta_despegue_aterrizaje_cusum(daDatos, tipo_test, umbral=10.0, show=False):
    ...
    daCorte = xr.apply_ufunc(detect_onset_aux, daDatos,
                   input_core_dims=[['time']],
                   output_core_dims=[['evento']],
                   #exclude_dims=set(('time',)),
                   vectorize=True,
...
def detecta_ini_mov(daDatos, tipo_test='CMJ', daPeso=None, daEventos=None, SDx=5, umbral=10.0, show=False):
    ...
    def detect_iniMov_peso_mayor_menor(data, peso, umbral_peso, iinianalisis, idespegue, win_up, win_down, ID):
        ...
        ini1 = detect_onset(-dat, threshold=-(peso-umbral_peso), n_above=int(win_down*daDatos.freq), threshold2=-(peso-umbral_peso)*1.2, n_above2=int(win_down*daDatos.freq)*0.5, show=show)
        if ini1.size!=0:
            #Pasada hacia atrás buscando ajuste fino que supera el peso
            ini2 = detect_onset(dat[ini1[0,0]:0:-1], threshold=peso, n_above=1, show=show)
            if ini2.size==0:
                ini2=np.array([[0,0]]) #si no encuentra, suma cero
            elif ini2[0,0]/daDatos.freq > 0.2:
                ini2[0,0]= int(0.2*daDatos.freq) #si la detección atrás se va muy lejos, retrocede un valor arbitrario de segundos
            ini_abajo = iinianalisis + ini1[0,0] - ini2[0,0] + 1 #+1 para coger el que ya ha pasado por debajo del peso
        else:
        ...
        # try:
        #Pasada inicial para ver cuándo baja por debajo del umbral peso+XSD
        ini1 = detect_onset(dat, threshold=(peso+umbral), n_above=int(win_up*daDatos.freq), show=show)
        if ini1.size!=0:
            #Pasada hacia atrás buscando ajuste fino que supera el peso
            ini2 = detect_onset(-dat[ini1[0,0]:0:-1], threshold=-peso, n_above=1, show=show)
            if ini2.size==0:
                ...
                ini2[0,0]= int(0.2*daDatos.freq) #si la detección atrás se va muy lejos, retrocede un valor arbitrario de segundos
            ini_arriba = iinianalisis + ini1[0,0] - ini2[0,0] + 1 #+1 para coger el que ya ha pasado por encima del peso
        else:
    ...
    if tipo_test=='DJ':
        ...
        iinianalisis = daEventos[3,2].sel(evento='iniAnalisis').data
        idespegue = daEventos[3,2].sel(evento='despegue').data
        ID = daEventos[3,2].ID.data
        """
        func_detect = detect_iniMov_peso_mayor_menor #detect_iniMov_peso_pcto
        daCorte = xr.apply_ufunc(func_detect, daDatos, daPeso.sel(ID=daDatos.ID, stat='media'),
                                                       daPeso.sel(ID=daDatos.ID, stat='sd')*SDx,
                                                       daEventos.sel(ID=daDatos.ID).sel(evento='iniAnalisis'),
                                                       daEventos.sel(ID=daDatos.ID).sel(evento='despegue'),
                                                       0.1, 0.05,
                                                       daDatos.ID,
                                   input_core_dims=[['time'], [], [], [], [], [], [], []],
                                   #output_core_dims=[['peso']],
        ...
        iinianalisis = daEventos[6,2].sel(evento='iniAnalisis').data
        idespegue = daEventos[6,2].sel(evento='despegue').data
        win_down = 0.05
        win_up = 0.1
        """
        func_detect = detect_iniMov_peso_mayor_menor #detect_iniMov_peso_pcto
        daCorte = xr.apply_ufunc(func_detect, daDatos, daPeso.sel(ID=daDatos.ID, stat='media'),
                                              ...
                                              daEventos.sel(ID=daDatos.ID).sel(evento='iniAnalisis'),
                                              daEventos.sel(ID=daDatos.ID).sel(evento='despegue'),
                                              0.05, 0.1, #ventanas down y up, en segundos
                                              daDatos.ID,
                                   input_core_dims=[['time'], [], [], [], [], [], [], []],
                                   #output_core_dims=[['peso']],
                                   #exclude_dims=set(('time',)),
        ...
        #Como no se observa un patrón estable, coge un tiempo prefijado antes del despegue, basado en la media de fase concéntrica en SJ (#((daEventosForces.sel(evento='finImpPos') - daEventosForces.sel(evento='iniImpPos'))/daSJ.freq).mean())
        daCorte = daEventos.sel(evento='despegue') - 0.25 *daDatos.freq
        ...
        def detect_iniMov_peso_pcto(data, peso, pcto_peso, idespegue):
            ...
            try:
                idespegue = int(idespegue)
                #Busca primer aterrizaje                
                aterr = detect_onset(-data[:idespegue], threshold=-umbral, n_above=int(0.05*daDatos.freq), show=show)[0,0]
                #Escanea desde el peso hacia abajo en porcentajes
                for pct in range(0, -pcto_peso, -1):
                    ini = detect_onset(data[aterr::-1], threshold=peso*(100-pct)/100, n_above=int(0.05*daDatos.freq), show=show)#
        ...
        idespegue = daEventos.sel(ID='S02_DJ_30', repe=1).sel(evento='despegue').data
        """
        daCorte = xr.apply_ufunc(detect_iniMov_peso_pcto, daDatos, daPeso.sel(stat='media'), SDx, daEventos.sel(evento='despegue'),
                                   input_core_dims=[['time'], [], [], []],
                                   #output_core_dims=[['peso']],
                                   #exclude_dims=set(('time',)),
                                   vectorize=True,
...
def detecta_fin_mov(daDatos, tipo_test, daPeso=None, daEventos=None, tipo_calculo='fuerza', SDx=2, show=False):
    """
    tipo_calculo puede ser 'velocidad' o 'fuerza'
    """
    ...
    def detect_onset_dif_v(data, umbral, sd, iaterrizaje, finAn, ID):
        if np.count_nonzero(~np.isnan(data))==0:
            return np.nan
        ...
        try:
            ...
            fin = detect_onset(dif, threshold=0.00005, n_above=int(0.05*daDatos.freq), show=show)
    ...
    def detect_onset_fuerza(data, umbral, sd, iaterrizaje, finAn, ID):
        #print(ID)            
        if np.count_nonzero(~np.isnan(data))==0:
            return np.nan
        
        try:
            fin = detect_onset(-data[int(finAn):int(iaterrizaje):-1], threshold=-umbral+sd*SDx, n_above=int(0.1*daDatos.freq), show=show)        
            fin = finAn - fin[-1,0] + 1 #+1 para coger el que ya ha superado el umbral
                               #fin[0,0] o fin[1,1] ?????           
        except:
            fin = finAn #len(data[~np.isnan(data)]) #por si no encuentra el criterio
        return float(fin)
    ...
    if tipo_calculo=='velocidad':
        f_calculo = detect_onset_dif_v
        datos = calcula_variables(daDatos, daPeso=daPeso, daEventos=daEventos.sel(evento=['iniMov', 'finAnalisis']))['v']
    elif tipo_calculo=='fuerza':
        f_calculo = detect_onset_fuerza
        datos = daDatos
    else:
        raise(f'Método de cálculo {tipo_calculo} no implementado')
    ...
    #data = daDatos[0,0].data
    data = datos[0,0].data
    iaterrizaje = daEventos.sel(evento='aterrizaje')[0,0].data
    finAn = daEventos.sel(evento='finAnalisis')[0,0].data
    ...
    daCorte = xr.apply_ufunc(f_calculo, datos, daPeso.sel(stat='media'), daPeso.sel(stat='sd'), daEventos.sel(evento='aterrizaje'), daEventos.sel(evento='finAnalisis'), daDatos.ID,
                   input_core_dims=[['time'], [], [], [], [], []],
                   #output_core_dims=[['peso']],
                   #exclude_dims=set(('time',)),
                   vectorize=True,
...
def detecta_fin_mov_convencional(daDatos, tipo_test, daPeso=None, daEventos=None, SDx=2):
    def detect_onset_aux(data, umbral, sd, iaterrizaje, ID):
        #print(ID)            
        if np.count_nonzero(~np.isnan(data))==0:
            return np.nan
        fin = detect_onset(-data[int(iaterrizaje):], threshold=-umbral+sd*SDx, n_above=int(0.1*daDatos.freq), show=False)
        try:
            fin = iaterrizaje + fin[0,1] + 1 #+1 para coger el que ya ha superado el umbral
                               #fin[0,0] o fin[1,1] ?????           
        except:
    ...
    if 'axis' in daDatos.dims:
        daDatos = daDatos.sel(axis='z')
    daCorte = xr.apply_ufunc(detect_onset_aux, daDatos, daPeso.sel(stat='media'), daPeso.sel(stat='sd'), daEventos.sel(evento='aterrizaje'), daDatos.ID,
                   input_core_dims=[['time'], [], [], [], []],
                   #output_core_dims=[['peso']],
                   #exclude_dims=set(('time',)),
                   vectorize=True,
    ...
    if tipo_test in ['SJ', 'SJPreac', 'CMJ', 'DJ', 'DJ2P']:
        def detect_onset_aux(data, ini, fin):
            try:
                ini=int(ini)
                fin=int(fin)
                ind = float(np.argmax(data[ini:fin]) + ini) #con -1 es el anterior a superar el umbral de 0. Coincide mejor con maxFz???
                # plt.plot(data[ini:fin])
                # plt.show()
        ...
        """
        daCorte = xr.apply_ufunc(detect_onset_aux, daDatos, daEventos.sel(evento='iniMov').data, daEventos.sel(evento='despegue').data,
                                   input_core_dims=[['time'], [], []],
                                   #output_core_dims=[['evento']],
...
def detecta_minFz(daDatos, tipo_test, daPeso=None, daEventos=None, umbral=10.0, show=False):
    ...
    if tipo_test=='CMJ':
        ...
        """       
        data = daDatos[0].data
        ini = daEventos[0].sel(evento='iniMov').data
        fin = daEventos[0].sel(evento='maxFlex').data
        """
        daCorte = xr.apply_ufunc(detect_onset_aux, daDatos, daEventos.sel(evento='iniMov').data, daEventos.sel(evento='maxFlex').data,
                                   input_core_dims=[['time'], [], []],
                                   #output_core_dims=[['evento']],
                                   #exclude_dims=set(('evento',)),
                                   vectorize=True,
        ...
        fin = daEventos[0,0].sel(evento='maxFz').data
        """
        daCorte = xr.apply_ufunc(detect_onset_aux, daDatos, daEventos.sel(evento='iniMov').data, daEventos.sel(evento='maxFz').data,
                                   input_core_dims=[['time'], [], []],
                                   #output_core_dims=[['evento']],
                                   #exclude_dims=set(('evento',)),
                                   vectorize=True,
        ...
        data= daDatos[0,1].data
        ini = daEventos[0,1].sel(evento='iniMov').data
        fin = daEventos[0,1].sel(evento='iniImpPos').data
        args_func_cortes = dict(threshold=-umbral, n_above=int(0.1*daDatos.freq), show=True)
        """
        daCorte = xr.apply_ufunc(detect_onset_aux, daDatos, daEventos.sel(evento='iniImpPos').data,
                       input_core_dims=[['time'],[]],
                       #output_core_dims=[['evento']],
                       #exclude_dims=set(('time',)),
                       vectorize=True,
    ...
    if tipo_test in ['SJ', 'SJPreac', 'CMJ', 'DJ']:
        def detect_onset_aux(data, peso, ini, fin):
            try:
                ini=int(ini)
                fin=int(fin)
                ini1 = detect_onset(data[ini:fin], threshold=peso, n_above=int(0.1*daDatos.freq), show=show)                
                ind = ini + ini1[-1] #se queda con el último paso por el peso, próximo al despegue
        ...
        ini = daEventos[5,0].sel(evento='iniMov').data
        fin = daEventos[5,0].sel(evento='despegue').data
        """
        ...
        def detect_onset_aux(data, peso, ini, fin):
            try:
                ...
                ini0 = detect_onset(data, threshold=30.0, n_above=int(0.1*daDatos.freq), show=False)[1,0]
                ini1 = detect_onset(data[ini0:fin], threshold=peso, n_above=int(0.1*daDatos.freq), show=False)
                ind = ini0 + ini1[0]
                ind[1] += 1 #+1 para coger el que ya ha pasado por debajo del peso
                #data[ind[0]-1] #peso
            except:
...
def detecta_max_flex(daDatos, tipo_test='CMJ', v=None, daPeso=None, daEventos=None):
    """
    ...
    if tipo_test in ['SJ', 'SJPreac', 'CMJ', 'DJ', 'DJ2P']:
        def detect_onset_aux(data, ini, fin):
            try:                
                ini=int(ini)
                fin=int(fin)
                ind = detect_onset(data[ini:fin], threshold=0, n_above=int(0.01*daDatos.freq), show=False) #los datos que llegan de velocidad están cortados desde el iniMov
                ind = ind[0,0] + ini
        ...
        ini = daEventos[0].sel(evento='iniMov').data
        fin = daEventos[0].sel(evento='despegue').data
        """
        daCorte = xr.apply_ufunc(detect_onset_aux, v, daEventos.sel(evento='iniImpPos'), daEventos.sel(evento='despegue'),
                                   input_core_dims=[['time'], [], []],
                                   #output_core_dims=[['peso']],
                                   #exclude_dims=set(('time',)),
                                   vectorize=True,
    ...
    if tipo_test=='DJ':
        ...
        def detect_onset_aux(data, peso, ini, fin):
            try:
                ini=int(ini)
                fin=int(fin)
                ind = detect_onset(data[ini:], threshold=peso, n_above=int(0.01*daDatos.freq), show=False)
                #ind += ini
                #data[int(ind)-1:int(ind)+2] #data[ind]
            except:
                ind = np.nan #por si no encuentra el criterio
        ...
        daCorte = xr.apply_ufunc(detect_onset_aux, daDatos.sel(axis='z'), daPeso.sel(stat='media').data, daInstantes.sel(evento='iniMov'), daInstantes.sel(evento='despegue'),
                                   input_core_dims=[['time'], [], [], []],
                                   #output_core_dims=[['peso']],
                                   #exclude_dims=set(('time',)),
                                   vectorize=True,
...
def detecta_inicio_cero(daDatos, umbral=100):
    ...
    def detecta_inicio_cero_aux(data, umbral):
        #print(ID)            
        if np.count_nonzero(~np.isnan(data))==0:
            return np.nan
        
        try:
            ind = detect_onset(data, threshold=umbral, n_above=int(0.1*daDatos.freq), show=False)        
    ...
    data = daDatos[0].data    
    """
    da = xr.apply_ufunc(detecta_inicio_cero_aux, daDatos, umbral,
                   input_core_dims=[['time'],[]],
                   #output_core_dims=[['peso']],
                   #exclude_dims=set(('time',)),
                   vectorize=True,
...
def detecta_final_cero(daDatos, umbral=100):
    """Detecta la parte final si se ha salido de la plataforma antes de tiempo.
    Se le deben pasar los archivos ya detectados como que tienen el final incorrecto.
    """
    if 'axis' in daDatos.dims:
    ...
    def detecta_final_cero_aux(data, umbral):
        #print(ID)            
        if np.count_nonzero(~np.isnan(data))==0:
            return np.nan
        try:
    ...
    da = xr.apply_ufunc(detecta_final_cero_aux, daDatos, umbral,
                   input_core_dims=[['time'],[]],
                   #output_core_dims=[['peso']],
                   #exclude_dims=set(('time',)),
                   vectorize=True,
...
def detecta_eventos_estandar(daDatos, daEventos=None, daPeso=None, tipo_test='CMJ', tipo_calculo_fin_mov='fuerza', umbral=30.0, SDx=5):
    if daPeso is None:
    ...
    daEventos.loc[dict(evento='iniMov')] = detecta_ini_mov(daDatos, tipo_test=tipo_test, daPeso=daPeso, daEventos=daEventos, umbral=umbral, SDx=SDx)#.sel(evento='despegue'))
    
    #Final del movimiento
    daEventos.loc[dict(evento='finMov')] = detecta_fin_mov(daDatos, tipo_test=tipo_test, daPeso=daPeso, daEventos=daEventos, tipo_calculo=tipo_calculo_fin_mov)#.sel(evento='aterrizaje'))
...
colr={'iniAnalisis':'grey', 'finAnalisis':'grey',
     'iniMov':'C0', 'finMov':'C0',
     'preactiv':'dodgerblue',
     'iniPeso':'deepskyblue', 'finPeso':'deepskyblue',
     #'iniPeso2':'deepskyblue', 'finPeso2':'deepskyblue', #para cuando se ponen a la vez el peso al inicio y al final
     'iniImpPos':'orange', 'finImpPos':'orange',
     'despegue':'r', 'aterrizaje':'r', 
     'maxFz':'brown', 'minFz':'cyan', 'maxFlex':'g'}

def completar_en_grafica_xr(g, ajusta_inifin, daPeso, daEventos):
            ...
            if isinstance(daEventos, xr.DataArray):
                for ev in daEventos.sel(dimensiones):#.evento:
                    ...
                    ax[i].text(ev / g.data.freq, y_texto, ev.evento.data,
                             ...
                             bbox=dict(facecolor=col, alpha=0.3, edgecolor='none', boxstyle='round,pad=0.3'+',rounding_size=.5'), transform=ax[i].transData)
            if ajusta_inifin:
...
def graficas_eventos(daDatos, daEventos=None, daPeso=None, n_en_bloque=4, show_in_console=True, ajusta_inifin=False, sharey=False, ruta_trabajo=None, nom_archivo_graf_global=None):
    """
    ...
    for n in range(0,len(daDatos.ID), distribuidor):
        ...
        g=dax.plot.line(x='time', alpha=0.8, aspect=1.5, sharey=sharey, **fils_cols) #, lw=1)
        completar_en_grafica_xr(g, ajusta_inifin, daPeso, daEventos)
        ...
        for ev in daEventos.sel(ID=ID, repe=repe).evento:
            ...
            #     print('\n',num)
                if not np.isnan(daEventos.sel(ID=ID, repe=repe, evento=ev)): #si existe el evento
                    plt.axvline(x=daEventos.sel(ID=ID, repe=repe, evento=ev)/daDatos.freq, c=col[str(ev.data)], lw=0.5, ls='--', dashes=(5, 5), dash_capstyle='round', alpha=0.5)
                    y_texto = plt.gca().get_ylim()[1] if str(ev.data) not in ['minFz', 'despegue', 'maxFz'] else plt.gca().get_ylim()[1]*0.8
                    plt.text(daEventos.sel(ID=ID, repe=repe, evento=ev).data/daDatos.freq, y_texto, ev.data,
            ...
            for i in range(len(ax)): #extrae cada axis (gráfica)                    
                ...
                if isinstance(daEventos, xr.DataArray):
                    for ev in daEventos.sel(dimensiones):#.evento:
                        ...
                        ax[i].axvline(x=ev/g.data.freq, c=colr[str(ev.evento.data)], lw=1, ls='--', dashes=(5, 5), dash_capstyle='round', alpha=0.7)
...
def recorta_ventana_analisis(daDatos, daEvents=None, ventana=None):
    """
    ...
    if ventana is not None:
        ...
        daIni = daEvents.isel(evento=0)
        daFin = daEvents.isel(evento=1)
        
    daCortado = xr.apply_ufunc(corta_ventana, daDatos, daIni, daFin, #.sel(ID=daDatos.ID, repe=daDatos.repe)
                   input_core_dims=[['time'], [], []],
                   output_core_dims=[['time']],
                   exclude_dims=set(('time',)),
                   vectorize=True,
...
def ajusta_offsetFz_vuelo_min(daDatos, tipo_test=None, umbral=20.0, pcto_ventana=5, show=False):
    ...
    if show:
        if 'plat' in daDatos.dims:
            if 'repe' in daDatos.dims:
                daReturn.stack(ID_repe=('ID', 'repe')).where(daReturn.stack(ID_repe=('ID', 'repe')) < offset.stack(ID_repe=('ID', 'repe'))).plot.line(x='time', col='ID_repe', hue='plat', col_wrap=4, alpha=0.7)
                #Sospechosos
                sospechosos = daReturn.stack(ID_repe=('ID', 'repe', 'plat')).where(np.abs(resta.stack(ID_repe=('ID', 'repe', 'plat'))) > umbral, drop=True)
                sospechosos.plot.line(x='time', col='ID_repe', hue='plat', col_wrap=4, alpha=0.7)
                print(f"Sospechosos {sospechosos.ID_repe.data}")
        else:
            if 'repe' in daDatos.dims:
                daReturn.stack(ID_repe=('ID', 'repe')).where(daReturn.stack(ID_repe=('ID', 'repe')) < offset.stack(ID_repe=('ID', 'repe'))).plot.line(x='time', col='ID', col_wrap=4)
            else:
                daReturn.where(daReturn < umbral*2.5).plot.line(x='time', col='ID', col_wrap=4)
...
def ajusta_offsetFz(daDatos, tipo_test=None, umbral=20.0, pcto_ventana=5, tipo_calculo='vuelo', show=False):
    """
    ...
    if tipo_calculo=='media':
        if tipo_test=='DJ2PApart':
            ...
            offset_plat2 = daReturn.sel(plat=2, time=slice(daReturn.time[-1]-1.5,daReturn.time[-1]-0.5)).mean('time')
            ...
            if show:
                if 'plat' in daDatos.dims:
                    if 'repe' in daDatos.dims:
                        daReturn.stack(ID_repe=('ID', 'repe')).where(daReturn.stack(ID_repe=('ID', 'repe')) < umbral).plot.line(x='time', col='ID_repe', hue='plat', col_wrap=4, alpha=0.7)
                        
                else:
                    if 'repe' in daDatos.dims:
                        daReturn.stack(ID_repe=('ID', 'repe')).where(daReturn.stack(ID_repe=('ID', 'repe')) < umbral).plot.line(x='time', col='ID', col_wrap=4)
                    else:
                        daReturn.where(daReturn < umbral).plot.line(x='time', col='ID', col_wrap=4)
    ...
    if tipo_calculo=='vuelo': #sin comprobar
        ...
        if tipo_test=='DJ2P':
            #busca despegue y aterrizaje provisionales
            vuelo = detecta_despegue_aterrizaje(daReturn, tipo_test, umbral=umbral, show=show)
        ...
        if show:
            if 'plat' in daDatos.dims:
                if 'repe' in daDatos.dims:            
                    offset_vuelo.plot.line(col='ID', col_wrap=4, hue='axis', sharey=False)
            else:
                recorta_ventana_analisis(daReturn, vuelo.sel(evento=['despegue', 'aterrizaje'])).plot.line(col='ID', col_wrap=4, hue='repe', sharey=False)
        ...
        if show:
            if 'plat' in daDatos.dims:
                if 'repe' in daDatos.dims:
                    daReturn.stack(ID_repe=('ID', 'repe')).where(daReturn.stack(ID_repe=('ID', 'repe')) < umbral*2.5).plot.line(x='time', col='ID_repe', hue='plat', col_wrap=4, alpha=0.7)
            else:
                daReturn.where(daReturn < umbral*2.5).plot.line(x='time', col='ID', col_wrap=4)
...
def ajusta_offsetFz_vuelo_convencional(daDatos, tipo_test='CMJ', umbral=20.0, pcto_ventana=5, show=False):
    #Ajusta buscando los vuelos concretamente
    if tipo_test=='DJ2P':
        ...
        if show:
            try: #comprobar si es necesario cuando hay ejes
                recorta_ventana_analisis(daDatos, vuelo.sel(evento=['despegue', 'aterrizaje'])).plot.line(col='ID', col_wrap=4, hue='axis', sharey=False)
            except:
...
def ajusta_offset_s(daDatos, tipo_test='DJ', umbral=20.0, pcto_ventana=5, show=False):
    def último_dato(data, ID):
        #print(ID)
        # plt.plot(data)            
    ...
    if 'axis' in daDatos.dims:
        daDatos = daDatos.sel(axis='z')
    daUltimo = xr.apply_ufunc(último_dato, daDatos, daDatos.ID,
                   input_core_dims=[['time'], []],
                   #output_core_dims=[['peso']],
                   #exclude_dims=set(('time',)),
                   vectorize=True,
...
def reset_Fz_vuelo(daDatos, tipo_test=None, umbral=20.0, pcto_ventana=5, show=False): #, ventana_vuelo=None):
    """
    ...
    if tipo_test=='DJ2PApart':
        ...
        vuelo = detecta_despegue_aterrizaje(daDatos, tipo_test, umbral=umbral)#, show=show)
        def detect_onset_aux(data, vuel, ID):
            if np.count_nonzero(~np.isnan(data))==0:
                return data
            # plt.plot(data)
            # plt.show()
        ...
        daReturn = xr.apply_ufunc(detect_onset_aux, daDatos, vuelo.sel(evento='despegue'), daDatos.ID,
                       input_core_dims=[['time'], [], []],
                       output_core_dims=[['time']],
                       #exclude_dims=set(('time',)),
                       vectorize=True,
    ...
    if show:
        if 'plat' in daDatos.dims:
            if 'repe' in daDatos.dims:
                daReturn.sel(axis='z').stack(ID_repe=('ID', 'repe')).where(daReturn.sel(axis='z').stack(ID_repe=('ID', 'repe')) < umbral*2.5).plot.line(x='time', col='ID_repe', hue='plat', col_wrap=4, alpha=0.7)
        else:
            daReturn.where(daReturn <= umbral*2.1).plot.line(x='time', col='ID', col_wrap=4)
...
def reset_F_vuelo_ejes(daDatos, tipo_test, umbral=20.0, pcto_ventana=5, show=False): #, ventana_vuelo=None):
    ...
    with xr.set_options(keep_attrs=True):
        daDatos = xr.where(~daDatos.isnull(), daDatos.where(daDatos > umbral, 0.0), daDatos)
        daDatos.time.attrs['units'] ='s' #por alguna razón lo cambiaba a newtons
        #daDatos.plot.line(row='ID', col='repe', hue='axis', sharey=False)
    
    if show:
        recorta_ventana_analisis(daDatos, vuelo+[-50, 50]).plot.line(col='ID', col_wrap=4, hue='axis', sharey=False)
    
    return daDatos

def reset_F_vuelo_ejes_convencional(daDatos, tipo_test, umbral=20.0, pcto_ventana=5, show=False): #, ventana_vuelo=None):
    if 'axis' in daDatos.dims:
        daDatosZ = daDatos.sel(axis='z')
    else:
        daDatosZ = daDatos
...
def calcula_variables(daDatos, daPeso=None, daEventos=None):
    ...
    return xr.Dataset({'BW':daBW, #F normalizada
                       'v':daV, 's':daS, 'P':daP, 'RFD':daRFD}
                      ).astype(daDatos.dtype).assign_attrs({'freq':daDatos.freq})
...
def calcula_results(daCinet=None, dsCinem=None, daPeso=None, daResults=None, daEventos=None):
    ...
    if not isinstance(daResults, xr.DataArray):
        daResults = (xr.full_like(daCinet.isel(time=0).drop_vars('time'), np.nan)
                             .expand_dims({'n_var':['tVuelo', 'tFaseInicioDesc', 'tFaseExc', 'tFaseConc', 
                                                    'FzMax', 'FzMin', 'FzTransicion',
                                                    'vDespegue', 'vAterrizaje', 'vMax', 'vMin',
                                                    'sIniMov', 'sFinMov', 'sDespegue', 'sAterrizaje', 'sDifDespAter', 'sMax', 'sMin', 
    ...
    daResults.loc[dict(n_var='vMax')] = recorta_ventana_analisis(dsCinem['v'], daEventos.sel(evento=['iniMov', 'despegue'])).max(dim='time')
    daResults.loc[dict(n_var='vMin')] = recorta_ventana_analisis(dsCinem['v'], daEventos.sel(evento=['iniMov', 'despegue'])).min(dim='time')
    
    #Posiciones
    daResults.loc[dict(n_var='sIniMov')] = dsCinem['s'].sel(time=daEventos.sel(evento='iniMov')/dsCinem.freq, method='nearest')
    daResults.loc[dict(n_var='sFinMov')] = dsCinem['s'].sel(time=(daEventos.sel(evento='finMov')-1)/dsCinem.freq, method='nearest')
    daResults.loc[dict(n_var='sDespegue')] = dsCinem['s'].sel(time=daEventos.sel(evento='despegue')/dsCinem.freq, method='nearest')
    daResults.loc[dict(n_var='sAterrizaje')] = dsCinem['s'].sel(time=daEventos.sel(evento='aterrizaje')/dsCinem.freq, method='nearest')
    daResults.loc[dict(n_var='sDifDespAter')] = daResults.loc[dict(n_var='sDespegue')] - daResults.loc[dict(n_var='sAterrizaje')]
    daResults.loc[dict(n_var='sMax')] = recorta_ventana_analisis(dsCinem['s'], daEventos.sel(evento=['despegue', 'aterrizaje'])).max(dim='time')
    daResults.loc[dict(n_var='sMin')] = recorta_ventana_analisis(dsCinem['s'], daEventos.sel(evento=['iniMov', 'despegue'])).min(dim='time')
    
    #Altura salto
    daResults.loc[dict(n_var='hTVuelo')] = g / 8 * daResults.loc[dict(n_var='tVuelo')]**2
    daResults.loc[dict(n_var='hVDespegue')] = daResults.loc[dict(n_var='vDespegue')]**2/(2*g)
    ...
    daResults.loc[dict(n_var='impNegDescenso')] = integra_completo(daCinet - 1, daEventos=daEventos.sel(evento=['iniMov', 'iniImpPos']))
    daResults.loc[dict(n_var='ImpPositDescenso')] = integra_completo(daCinet - 1, daEventos=daEventos.sel(evento=['iniImpPos', 'maxFlex']))
...
def calcula_results_EMG(daEMG=None, daResults=None, daEventos=None):
    if not isinstance(daResults, xr.DataArray):
        if 'axis' in daEventos.coords:
            daEventos = daEventos.drop_vars('axis')
        daResults = (xr.full_like(daEMG.isel(time=0).drop_vars(['time', 'axis']), np.nan)
                             .expand_dims({'n_var':['EMGPreIniMean', 'EMGExcMean', 'EMGConcMean', 'EMGVueloMean', 
                                                    'EMGPreIniInteg', 'EMGExcInteg', 'EMGConcInteg', 'EMGVueloInteg',
                                                    'EMGPreIniRMS', 'EMGExcRMS', 'EMGConcRMS', 'EMGVueloRMS',
                                                    ]
                                           },
                                          axis=-1)
                            ).copy()
    ...
    #Medias
    daResults.loc[dict(n_var='EMGPreIniMean')] = recorta_ventana_analisis(daDatos=daEMG, daEvents=daEventos.sel(evento=['preactiv', 'iniMov'])).mean('time')
    daResults.loc[dict(n_var='EMGExcMean')] = recorta_ventana_analisis(daEMG, daEventos.sel(evento=['iniMov', 'maxFlex'])).mean('time')
    daResults.loc[dict(n_var='EMGConcMean')] = recorta_ventana_analisis(daEMG, daEventos.sel(evento=['maxFlex', 'despegue'])).mean('time')
    daResults.loc[dict(n_var='EMGVueloMean')] = recorta_ventana_analisis(daEMG, daEventos.sel(evento=['despegue', 'aterrizaje'])).mean('time')
    
    #Integrales
    daResults.loc[dict(n_var='EMGPreIniInteg')] = integra_completo(daEMG, daEventos=daEventos.sel(evento=['preactiv', 'iniMov']))    
    daResults.loc[dict(n_var='EMGExcInteg')] = integra_completo(daEMG, daEventos=daEventos.sel(evento=['iniMov', 'maxFlex']))
    daResults.loc[dict(n_var='EMGConcInteg')] = integra_completo(daEMG, daEventos=daEventos.sel(evento=['maxFlex', 'despegue']))
    daResults.loc[dict(n_var='EMGVueloInteg')] = integra_completo(daEMG, daEventos=daEventos.sel(evento=['despegue', 'aterrizaje']))
      
    #RMS
    daResults.loc[dict(n_var='EMGPreIniRMS')] = RMS(daDatos=daEMG, ventana=daEventos.sel(evento=['preactiv', 'iniMov']))
    daResults.loc[dict(n_var='EMGExcRMS')] = RMS(daDatos=daEMG, ventana=daEventos.sel(evento=['iniMov', 'maxFlex']))
    daResults.loc[dict(n_var='EMGConcRMS')] = RMS(daEMG, ventana=daEventos.sel(evento=['maxFlex', 'despegue']))
    daResults.loc[dict(n_var='EMGVueloRMS')] = RMS(daEMG, ventana=daEventos.sel(evento=['despegue', 'aterrizaje']))
...
class trata_fuerzas_saltos:
    ...
    def calcula_peso(self, ventana=[100, 600], show=False):
        self.peso = self.datos.sel(axis='z').isel(time=slice(ventana[0], ventana[1])).mean(dim='time')
        
        if show:
            def dibuja_peso(x,y, **kwargs): #de momento no funciona
                print(x)#kwargs['data'])
                #plt.plot()
            g=self.datos.sel(axis='z').plot.line(col='ID', col_wrap=4, hue='trial', sharey=False)
            #g = xr.plot.FacetGrid(self.datos, col='ID', col_wrap=4)
            #g.map_dataarray(dibuja_peso, x='time', y=None)#, y='trial')
            ...
            for h, ax in enumerate(g.axes): #extrae cada fila
                for i in range(len(ax)): #extrae cada axis (gráfica)     
                    try:
                        ...
                        #Rango medida peso
                        #ax[i].axvspan(g.data.time[int(ventana[0]*self.datos.freq)], g.data.time[int(ventana[1]*self.datos.freq)], alpha=0.2, color='C1')
                        ax[i].axvspan((len(self.datos.time) + ventana[0])/self.datos.freq, (len(self.datos.time) + ventana[1])/self.datos.freq, alpha=0.2, color='C1')
                        #Líneas peso
...
if __name__ == "__main__":
    if False:
        ...
        ruta =  Path(r'F:\Investigacion\Proyectos\Saltos\2023PreactivacionSJ\DataCollection\S01\FeedbackFuerza')
        ...
        daFuerzas = load_merge_bioware_pl(ruta, estudio='preacSJ23', n_vars_load=['abs time (s)', 'Fx', 'Fy', 'Fz', 'Fz1', 'Fz2', 'Fz3', 'Fz4', 'Ax', 'Ay'])
        daFuerzasrepe = separa_dim_repe(daFuerzas)
        daFuerzasrepeaxis = separa_dim_axis(daFuerzasrepe.sel(n_var=['Fx', 'Fy', 'Fz']))
        
        
        daFuerzas = load_merge_bioware_pl(ruta, estudio='preacSJ23')
        ...
        daCMJ = ajusta_offsetFz(daDatos=daCMJ, tipo_test='CMJ', tipo_calculo='vuelo', umbral=40, pcto_ventana=5)#, show=True)
        
        #Sustitución ventana vuelo por cero-------------
        daCMJ = reset_Fz_vuelo(daDatos=daCMJ, tipo_test='CMJ', umbral=30, pcto_ventana=5)#, show=True)
        ...
        daEventosForces.loc[dict(evento=['iniAnalisis', 'finAnalisis'])] = \
                                estima_inifin_analisis(daDatos=daCMJ, tipo_test='CMJ',
                                                              daEventos=daEventosForces.sel(evento=['iniAnalisis', 'finAnalisis']),
                                                              ventana=[1.5, 1.2], umbral=30
                                                              )
        #Ajuste personalizado inicio-fin análisis
        #daEventosForces.loc[dict(ID='S06_CMJ_2', repe=0, evento='iniAnalisis')] = daEventosForces.loc[dict(ID='S06_CMJ_2', repe=0, evento='iniAnalisis')] - np.array(0.3) * daCMJ.freq
        
        #----Cálculo peso
        #Ventanas donde calcular el peso en cada salto
        daEventosForces.loc[dict(evento=['iniPeso', 'finPeso'])] = np.array([0., 0.5]) * daCMJ.freq
        #daVentanasPeso = daEventosForces.loc[dict(evento=['iniPeso', 'finPeso'])]
        """Comprobación
        ...
        #daEventosForces.loc[dict(ID='S04_CMJ_2', repe=[1,2], evento=['iniPeso', 'finPeso'])] = np.array([0.1, 1.1]) * daCMJ.freq
        
        
        #Primero calcula el peso de la media de la zona estable seleccionada
        daPeso_media = calcula_peso(daDatos=daCMJ, ventana_peso=daEventosForces.sel(evento=['iniPeso', 'finPeso']))#, show=True)
        #Para afinar el peso, primero detecta eventos provisionales
        daEventosForces = detecta_eventos_estandar(daDatos=daCMJ, daEventos=daEventosForces, daPeso=daPeso_media, tipo_test='CMJ', umbral=30.0)
        daPesoCMJ = afina_peso(daDatos=daCMJ, daPeso=daPeso_media, daEventos=daEventosForces, tipo_calculo='iter')#, show=True)
